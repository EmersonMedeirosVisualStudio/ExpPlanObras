...
function isTrialExpired(tenant, now) {
    return ((tenant.subscriptionStatus || 'TRIAL') === 'TRIAL' &&
        !!tenant.trialEndsAt &&
    ...
    try {
        ...
        if (subStatus === 'ACTIVE') {
            ...
            if (tenant.paidUntil < now) {
                ...
                if (now <= graceEndsAt) {
                    if (tenant.subscriptionStatus !== 'GRACE_PERIOD' || !tenant.gracePeriodEndsAt) {
                        await prisma.tenant.update({
                            where: { id: tenant.id },
                            data: { subscriptionStatus: 'GRACE_PERIOD', gracePeriodEndsAt: graceEndsAt },
                        });
                ...
                await prisma.tenant.update({
                    where: { id: tenant.id },
                    data: { subscriptionStatus: 'EXPIRED', gracePeriodEndsAt: null },
                });
                await prisma.tenantHistoryEntry.create({
                    data: {
                        tenantId: tenant.id,
        ...
        for (const a of abrangencias) {
            ...
            if (tipo === 'OBRA' && typeof a.obraId === 'number' && a.obraId > 0)
                obras.add(a.obraId);
            if (tipo === 'UNIDADE' && typeof a.unidadeId === 'number' && a.unidadeId > 0)
...
export async function checkSystemAdmin(request, reply) {
    try {
        await request.jwtVerify();

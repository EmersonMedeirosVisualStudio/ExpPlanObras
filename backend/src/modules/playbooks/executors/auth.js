...
export const authExecutors = [
    {
        ...
        async execute(input) {
            ...
            if (!userId)
                return { ok: false, error: 'userId ausente' };
            const tu = await prisma.tenantUser.findUnique({ where: { tenantId_userId: { tenantId: input.tenantId, userId } } }).catch(() => null);
            if (!tu)
                return { ok: false, error: 'tenantUser não encontrado' };
            const now = new Date();
            await prisma.tenantUser.update({ where: { id: tu.id }, data: { tokenRevokedBefore: now } });
    ...
    },
    {
        ...
        async execute(input) {
            ...
            await prisma.tenantUser.update({ where: { id: tu.id }, data: { bloqueado: true, bloqueadoAteEm: until, tokenRevokedBefore: now } });

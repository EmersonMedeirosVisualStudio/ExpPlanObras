...
function toRisk(v) {
    ...
    if (s === 'MEDIO' || s === 'ALTO' || s === 'CRITICO')
        return s;
    return 'BAIXO';
}
function toApprovalPolicy(v) {
    const s = String(v || '').toUpperCase();
    if (s === 'NAO_EXIGE' || s === 'EXIGE_ANTES' || s === 'EXIGE_SE_RISCO_ALTO' || s === 'QUATRO_OLHOS')
        return s;
    return 'EXIGE_SE_RISCO_ALTO';
}
function toMode(v) {
    const s = String(v || '').toUpperCase();
    if (s === 'SEMI_AUTOMATICO' || s === 'AUTOMATICO')
        return s;
    return 'MANUAL';
}
...
async function executarPasso(args) {
    const exec = getPlaybookActionExecutor(String(args.passo.tipoAcao));
    ...
    if (!exec) {
        ...
        return { ok: false, error: 'EXECUTOR_NAO_SUPORTADO', incidenteId: args.incidenteId ?? null };
    }
...
async function runExecucao(args) {
    ...
    if (!ex || ex.tenantId !== args.tenantId)
        return { ok: false, reason: 'EXECUCAO_INVALIDA' };
    if (ex.statusExecucao !== 'EXECUTANDO') {
        await prisma.observabilidadePlaybookExecucao.update({ where: { id: ex.id }, data: { statusExecucao: 'EXECUTANDO', iniciadoEm: ex.iniciadoEm ?? new Date() } });
    }
    const pb = ex.playbook;
    let incidenteId = ex.incidenteId ?? null;
    let anyFail = false;
    let executed = 0;
    for (const passo of pb.passos) {
        ...
        if (!r.ok) {
            anyFail = true;
            if (!passo.continuaEmErro)
                break;
        }
    }
    const finalStatus = anyFail ? (executed === pb.passos.length ? 'PARCIAL' : 'FALHA') : 'CONCLUIDA';
    await prisma.observabilidadePlaybookExecucao.update({
        where: { id: ex.id },
        data: { statusExecucao: finalStatus, finalizadoEm: new Date(), resultadoResumoJson: redactPayload({ anyFail, executed, total: pb.passos.length, incidenteId }) },
    });
    await emitObservabilityEvent({
        tenantId: args.tenantId,
    ...
    return { ok: true, execucaoId: ex.id, statusExecucao: finalStatus, incidenteId, anyFail };
}
export async function executarPlaybook(args) {
    const pb = await prisma.observabilidadePlaybook.findUnique({
        where: { id: args.playbookId },
        include: { passos: { orderBy: [{ ordemExecucao: 'asc' }] } },
    });
    ...
    if (existing && existing.statusExecucao !== 'CANCELADA') {
        return { ok: true, execucaoId: existing.id, statusExecucao: existing.statusExecucao, aprovacaoExigida: existing.aprovacaoExigida };
    }
    const status = sim.approvalRequired ? 'PENDENTE_APROVACAO' : 'EXECUTANDO';
    ...
    if (sim.approvalRequired) {
        return { ok: true, execucaoId: created.id, statusExecucao: created.statusExecucao, aprovacaoExigida: true };
    }
    const ran = await runExecucao({ tenantId: args.tenantId, execucaoId: created.id, executorUserId: args.executorUserId });
    if (!ran.ok)
        return ran;
    return { ok: true, execucaoId: created.id, statusExecucao: ran.statusExecucao, aprovacaoExigida: false, incidenteId: ran.incidenteId };
}
export async function aprovarExecucao(args) {
    const ex = await prisma.observabilidadePlaybookExecucao.findUnique({ where: { id: args.execucaoId } }).catch(() => null);
    if (!ex || ex.tenantId !== args.tenantId)
        return { ok: false, reason: 'EXECUCAO_INVALIDA' };
    if (ex.statusExecucao !== 'PENDENTE_APROVACAO')
    ...
    if (String(pb.politicaAprovacao || '').toUpperCase() === 'QUATRO_OLHOS' && ex.executadoPorUserId && ex.executadoPorUserId === args.aprovadorUserId) {
        return { ok: false, reason: 'QUATRO_OLHOS' };
    }
    await prisma.observabilidadePlaybookExecucao.update({
        where: { id: ex.id },
        data: { aprovadoPorUserId: args.aprovadorUserId, aprovadoEm: new Date(), statusExecucao: 'EXECUTANDO', iniciadoEm: new Date() },
    });
    ...
    return { ok: true, triggered: ran };
}
export async function cancelarExecucao(args) {
    const ex = await prisma.observabilidadePlaybookExecucao.findUnique({ where: { id: args.execucaoId } }).catch(() => null);
    if (!ex || ex.tenantId !== args.tenantId)
        return { ok: false, reason: 'EXECUCAO_INVALIDA' };
    if (ex.statusExecucao === 'CONCLUIDA' || ex.statusExecucao === 'CANCELADA')
        return { ok: false, reason: 'STATUS_INVALIDO' };
    await prisma.observabilidadePlaybookExecucao.update({
        where: { id: ex.id },
        data: { statusExecucao: 'CANCELADA', finalizadoEm: new Date(), resultadoResumoJson: redactPayload({ canceladoPor: args.userId, motivo: args.motivo ?? null }) },
    });
    await emitObservabilityEvent({
        tenantId: args.tenantId,
        categoria: 'SECURITY',

...
export default async function playbooksRoutes(server) {
    ...
    server.get('/playbooks/:id', async (request, reply) => {
        ...
        if (!pb || pb.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Playbook não encontrado');
        return ok(reply, pb);
    });
    server.put('/playbooks/:id', {
        schema: {
    ...
    server.post('/playbooks/:id/executar', {
        schema: {
            params: z.object({ id: z.coerce.number().int() }),
            body: z
                .object({
    ...
    server.get('/playbooks/execucoes', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusExecucao = String(q.status).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.post('/playbooks/execucoes/:id/aprovar', { schema: { params: z.object({ id: z.coerce.number().int() }) } }, async (request, reply) => {
        const ctx = await requireAdmin(request, reply);
        if (!ctx || ctx.success === false)
            return;
        const { id } = request.params;
        ...
        if (!res.ok)
            return fail(reply, 400, res.reason || 'Falha ao aprovar');
        return ok(reply, res, { message: 'Execução aprovada' });
    });
    server.post('/playbooks/execucoes/:id/cancelar', { schema: { params: z.object({ id: z.coerce.number().int() }), body: z.object({ motivo: z.string().optional().nullable() }) } }, async (request, reply) => {
        const ctx = await requireAdmin(request, reply);
        if (!ctx || ctx.success === false)
            return;
        const { id } = request.params;
        ...
        if (!res.ok)
            return fail(reply, 400, res.reason || 'Falha ao cancelar');
    ...
    server.get('/incidentes/:id/timeline', async (request, reply) => {
        ...
        if (!inc || inc.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Incidente não encontrado');
    ...
    server.post('/incidentes/:id/timeline', {
        schema: {
            params: z.object({ id: z.coerce.number().int() }),
            body: z.object({
    ...
    server.get('/compliance/casos', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusCaso = String(q.status).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.get('/compliance/casos/:id', async (request, reply) => {
        ...
        if (!row || row.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Caso não encontrado');
        return ok(reply, row);
    });
    server.post('/compliance/casos/:id/evidencias', {
        schema: {
            params: z.object({ id: z.coerce.number().int() }),
            body: z.object({
        ...
        if (!caso || caso.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Caso não encontrado');
        const body = request.body;
    ...
    server.post('/compliance/casos/:id/encerrar', { schema: { params: z.object({ id: z.coerce.number().int() }), body: z.object({ parecerFinal: z.string().min(3) }) } }, async (request, reply) => {
        const ctx = await requireAdmin(request, reply);
        if (!ctx || ctx.success === false)
            return;
        const { id } = request.params;
        ...
        if (!caso || caso.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Caso não encontrado');
        await prisma.observabilidadeCasoCompliance.update({ where: { id: caso.id }, data: { statusCaso: 'CONCLUIDO', parecerFinal: String(body.parecerFinal) } });

...
export function getPlanConfig(plan) {
    ...
    if (plan === 'ANNUAL') {
        return { plan, months: 12, amount: annualPrice, currency: 'BRL' };
    }
    const biennial = Math.round(annualPrice * 2 * 0.85 * 100) / 100;
    return { plan, months: 24, amount: biennial, currency: 'BRL' };
}
...
function getNotificationUrl() {
    ...
    if (url.startsWith('http')) {
        if (url.includes('/api/webhooks/mercadopago'))
            return url;
...
export async function createTenantCheckout(tenantId, payerEmail, plan) {
...
export async function syncTenantFromPreapproval(preapprovalId) {
    const token = getAccessToken();
    ...
    if (update.subscriptionStatus) {
        const msg = update.subscriptionStatus === 'ACTIVE'

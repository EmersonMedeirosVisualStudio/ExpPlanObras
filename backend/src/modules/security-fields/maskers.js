...
function maskHashShort(v) {
    const s = String(v || '').trim();
    if (s.length <= 10)
        return '***';
    return `${s.slice(0, 4)}...${s.slice(-4)}`;
}
function maskNameInitials(v) {
    const s = String(v || '').trim();
    if (!s)
        return '';
...
export function applyMask(strategy, value) {

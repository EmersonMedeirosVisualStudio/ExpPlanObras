...
async function requireSecurityAdmin(request, reply) {
...
export default async function securityFieldsRoutes(server) {
    ...
    server.get('/policies', async (request, reply) => {
        ...
        if (query.recurso)
            where.recurso = query.recurso;
        if (query.acao)
            where.acao = query.acao;
        if (query.ativo === 'true')
            where.ativo = true;
    ...
    server.post('/simulate', {
        ...
        const preview = await sanitizeResourceObject(body.sample, { tenantId: ctx.tenantId, userId, resource: body.resource, action: body.action, entityId: body.entityId ?? null, exportacao: body.action === 'EXPORT' }, subject);

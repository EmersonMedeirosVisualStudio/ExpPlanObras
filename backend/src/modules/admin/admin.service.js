...
export async function createTenantByAdmin(input) {
    const { name, slug, cnpj, companyEmail, companyWhatsapp, link, street, number, neighborhood, city, state, cep, latitude, longitude, representativeName, representativeEmail, representativeCpf, representativePassword, representativeWhatsapp } = input;
...
export async function updateTenant(id, input) {
    ...
    if (Object.prototype.hasOwnProperty.call(input, 'trialEndsAt') && input.trialEndsAt !== undefined) {
        data.trialEndsAt = input.trialEndsAt === null ? null : new Date(input.trialEndsAt);
    }
    if (Object.prototype.hasOwnProperty.call(input, 'paidUntil') && input.paidUntil !== undefined) {
        data.paidUntil = input.paidUntil === null ? null : new Date(input.paidUntil);
    }
...
export async function manualGrantTenantAccess(id, input) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        const base = new Date(Math.max(now.getTime(), tenant.paidUntil ? tenant.paidUntil.getTime() : 0, tenant.gracePeriodEndsAt ? tenant.gracePeriodEndsAt.getTime() : 0));
...
export async function revokeManualTenantAccess(id, input) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (!isManual)
            throw new Error('Somente liberações manuais podem ser revogadas');
...
export async function acceptClaimAsAdmin(input) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (tenant.companyEmail)
            emails.add(normalizeEmail(String(tenant.companyEmail)));
        ...
        const now = new Date();
        const base = new Date(Math.max(now.getTime(), tenant.paidUntil ? new Date(tenant.paidUntil).getTime() : 0, tenant.gracePeriodEndsAt ? new Date(tenant.gracePeriodEndsAt).getTime() : 0));
        const paidUntil = new Date(base);
...
export async function activateTenantSubscription(id, months) {
    return prisma.$transaction(async (tx) => {
        const tenant = await tx.tenant.findUnique({
            where: { id },
...
export async function grantTenantAccessDays(id, days) {
    return manualGrantTenantAccess(id, { reason: 'PAYMENT', days });
}
export async function resetRepresentativePassword(tenantId, newPassword) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (!rep) {
            ...
            if (!link?.user)
                throw new Error('Representante da empresa não encontrado.');
        ...
        if (!user)
            throw new Error('Usuário do representante não encontrado.');

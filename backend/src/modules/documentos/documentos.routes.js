...
function baseApiUrlFromEnvOrRequest(request) {
...
function parseIdOrFail(reply, params, notFoundMessage) {
...
async function toBufferFromMultipart(filePart) {
    if (!filePart)
        return Buffer.from([]);
    if (typeof filePart.toBuffer === 'function')
        return (await filePart.toBuffer());
...
export default async function documentosRoutes(server) {
    ...
    server.get('/', async (request, reply) => {
        ...
        if (categoriaPrefix)
            whereBase.categoriaDocumento = { startsWith: categoriaPrefix };
        let where = whereBase;
    ...
    server.post('/', {
        ...
        if (entidadeTipo && !obraId && !contratoId)
            return fail(reply, 400, 'Entidade inválida');
    ...
    server.put('/:id', {
        ...
        const descricaoDocumento = body.descricaoDocumento != null ? String(body.descricaoDocumento).trim() : null;
        if (tituloDocumento !== null && !tituloDocumento)
            return fail(reply, 400, 'Título inválido');
    ...
    server.delete('/:id', async (request, reply) => {
        ...
        if (!doc || doc.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Documento não encontrado');
        await prisma.documento.update({ where: { id: doc.id }, data: { statusDocumento: 'CANCELADO', updatedAt: new Date() } });
    ...
    server.get('/:id/download', async (request, reply) => {
        ...
        if (!versaoId)
            return fail(reply, 404, 'Documento sem versão');
    ...
    server.post('/:id/versoes', async (request, reply) => {
        ...
        if (!filePart)
            return fail(reply, 400, 'Arquivo obrigatório');
        ...
        await prisma.$transaction([
            prisma.documentoVersao.update({ where: { id: created.id }, data: { urlOriginal, updatedAt: new Date() } }),
    ...
    server.get('/versoes/:id', async (request, reply) => {
        ...
        if (!v || v.tenantId !== ctx.tenantId || v.documento.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Versão não encontrada');
    ...
    server.put('/versoes/:id/fluxo', {
        ...
        if (!v || v.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Versão não encontrada');
        await prisma.documentoVersao.update({ where: { id }, data: { fluxoJson: body.itens, updatedAt: new Date() } });
        return ok(reply, { ok: true });
    });
    server.post('/versoes/:id/acoes', {
        schema: {
            body: z.object({
                acao: z.string().min(2),
        ...
        if (acao === 'GERAR_PDF_FINAL') {
            ...
            if (!blob)
                return fail(reply, 404, 'Arquivo indisponível');
        ...
        const assinaturas = Array.isArray(v.assinaturasJson) ? v.assinaturasJson : [];
        ...
        await prisma.documentoVersao.update({ where: { id }, data: { assinaturasJson: assinaturas, updatedAt: new Date() } });
        return ok(reply, { ok: true });
    });
    async function buildVerificacaoDTO(versao, documento) {
        const blob = versao.conteudoPdfCarimbado || versao.conteudoOriginal;
        const buf = blob ? Buffer.from(blob) : null;
        const hashConferido = buf ? sha256Hex(buf) : null;
        const hashEsperado = versao.conteudoPdfCarimbado ? (versao.hashSha256PdfCarimbado || null) : (versao.hashSha256Original || null);
        const assinaturas = Array.isArray(versao.assinaturasJson) ? versao.assinaturasJson : [];
        return {
    ...
    server.get('/verificacao/:token', async (request, reply) => {
        ...
        if (!v)
            return fail(reply, 404, 'Token inválido');
        return ok(reply, await buildVerificacaoDTO(v, v.documento));
    });
    server.get('/versoes/:id/download', async (request, reply) => {
        ...
        if (!id)
            return;
        const q = z.object({ tipo: z.string().optional() }).parse(request.query || {});

...
import { createContrato, createContratoAditivo, createContratoServico, createCronogramaDependencia, cancelarContratoAditivo, addContratoEventoAnexo, createContratoMedicao, createContratoPagamento, createContratoProgramacaoFinanceira, createSubcontrato, deleteContratoPagamento, deleteContratoProgramacaoFinanceira, deleteSubcontrato, deleteCronogramaDependencia, getContratoConsolidado, getContratoById, getContratoCronograma, getContratosDashboard, getContratosFaturamento, getSubcontratosResumo, createContratoObservacao, downloadContratoEventoAnexo, listContratoAditivos, listContratoEventos, listContratoMedicoes, listContratoPagamentos, listContratoProgramacaoFinanceira, listContratoServicos, listSubcontratos, listContratos, aprovarContratoAditivo, seedCronogramaFromServicos, updateContrato, updateContratoAditivo, updateContratoMedicaoStatus, updateContratoProgramacaoFinanceira, updateCronogramaItemDatas, updateSubcontrato, } from './contratos.service.js';
export default async function contratosRoutes(server) {
    ...
    server.get('/realtime/stream', {
        ...
        const { topics, contratoId } = request.query;
        const topicList = String(topics || '')
            .split(',')
            .map((t) => t.trim())
            .filter(Boolean);
        if (contratoId)
            topicList.push(`contrato:${Number(contratoId)}`);
        if (!topicList.length)
            topicList.push('contratos');
        ...
        const unsubs = topicList.map((t) => subscribe(t, (msg) => {
            send(msg);
        }));
    ...
    server.get('/:id', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        const row = await getContratoById(tenantId, id);
        if (!row)
            return reply.code(404).send({ message: 'Contrato não encontrado' });
        return reply.send(row);
    });
    server.get('/:id/consolidado', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        try {
    ...
    server.get('/:id/subcontratos/resumo', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        try {
    ...
    server.get('/:id/subcontratos', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        try {
    ...
    server.post('/:id/subcontratos', {
        ...
        const { id } = request.params;
        try {
            const created = await createSubcontrato(tenantId, id, request.body);
    ...
    server.put('/:id/subcontratos/:subId', {
        ...
        try {
            const updated = await updateSubcontrato(tenantId, id, subId, request.body);
    ...
    server.get('/:id/medicoes', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        try {
    ...
    server.post('/:id/medicoes', {
        ...
        const { id } = request.params;
        try {
            const created = await createContratoMedicao(tenantId, id, request.body);
            return reply.send(created);
        }
        catch (e) {
    ...
    server.put('/:id/medicoes/:medicaoId', {
        schema: {
        ...
        const tenantId = request.user.tenantId;
        const { id, medicaoId } = request.params;
        try {
            const updated = await updateContratoMedicaoStatus(tenantId, id, medicaoId, request.body);
            return reply.send(updated);
        }
        catch (e) {
    ...
    server.get('/:id/pagamentos', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        try {
    ...
    server.post('/:id/pagamentos', {
        ...
        const { id } = request.params;
        try {
            const created = await createContratoPagamento(tenantId, id, request.body);
            return reply.send(created);
        }
        catch (e) {
    ...
    server.delete('/:id/pagamentos/:pagamentoId', { schema: { params: z.object({ id: z.coerce.number().int().positive(), pagamentoId: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id, pagamentoId } = request.params;
        try {
    ...
    server.get('/:id/programacao-financeira', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        try {
    ...
    server.post('/:id/programacao-financeira', {
        schema: {
            params: z.object({ id: z.coerce.number().int().positive() }),
            body: z.object({
        ...
        const { id } = request.params;
        try {
            const created = await createContratoProgramacaoFinanceira(tenantId, id, request.body);
            return reply.send(created);
        }
        catch (e) {
    ...
    server.put('/:id/programacao-financeira/:itemId', {
        schema: {
        ...
        try {
            const updated = await updateContratoProgramacaoFinanceira(tenantId, id, itemId, request.body);
            return reply.send(updated);
        }
        catch (e) {
    ...
    server.post('/:id/observacoes', {
        ...
        const { id } = request.params;
        try {
            const created = await createContratoObservacao(tenantId, id, { ...request.body, actorUserId: userId ?? null });
            return reply.send(created);
        }
        catch (e) {
    ...
    server.post('/:id/eventos/:eventoId/anexos', {
        ...
        const tenantId = request.user.tenantId;
        const userId = request.user.userId;
        const { id, eventoId } = request.params;
        try {
            const created = await addContratoEventoAnexo(tenantId, id, eventoId, { ...request.body, actorUserId: userId ?? null });
            return reply.send(created);
        }
        catch (e) {
    ...
    server.get('/:id/eventos/:eventoId/anexos/:anexoId', { schema: { params: z.object({ id: z.coerce.number().int().positive(), eventoId: z.coerce.number().int().positive(), anexoId: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id, eventoId, anexoId } = request.params;
        try {
    ...
    server.get('/:id/aditivos', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        try {
    ...
    server.post('/:id/aditivos', {
        ...
        const { id } = request.params;
        try {
            const created = await createContratoAditivo(tenantId, id, request.body);
            return reply.send(created);
        }
        catch (e) {
    ...
    server.put('/:id/aditivos/:aditivoId', {
        ...
        const tenantId = request.user.tenantId;
        const { id, aditivoId } = request.params;
        try {
            const updated = await updateContratoAditivo(tenantId, id, aditivoId, request.body);
            return reply.send(updated);
        }
        catch (e) {
    ...
    server.get('/:id/servicos', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        const rows = await listContratoServicos(tenantId, id);
        return reply.send(rows);
    });
    server.post('/:id/servicos', {
        schema: {
            params: z.object({ id: z.coerce.number().int().positive() }),
            body: z.object({
        ...
        const { id } = request.params;
        try {
            const created = await createContratoServico(tenantId, id, request.body);
            return reply.send(created);
        }
        catch (e) {
    ...
    server.get('/:id/cronograma', { schema: { params: z.object({ id: z.coerce.number().int().positive() }) } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        try {
    ...
    server.post('/:id/cronograma/seed', {
        schema: {
            params: z.object({ id: z.coerce.number().int().positive() }),
            body: z
                .object({
        ...
        const { id } = request.params;
        try {
            const data = await seedCronogramaFromServicos(tenantId, id, request.body);
            return reply.send(data);
        }
        catch (e) {
    ...
    server.put('/:id/cronograma/:itemId', {
        schema: {
            params: z.object({ id: z.coerce.number().int().positive(), itemId: z.coerce.number().int().positive() }),
        ...
        const { id, itemId } = request.params;
        try {
            const updated = await updateCronogramaItemDatas(tenantId, id, itemId, request.body);
            return reply.send(updated);
        }
        catch (e) {
    ...
    server.post('/:id/cronograma/dependencias', {
        schema: {
            params: z.object({ id: z.coerce.number().int().positive() }),
            body: z.object({
        ...
        const { id } = request.params;
        try {
            const created = await createCronogramaDependencia(tenantId, id, request.body);
            return reply.send(created);
        }
        catch (e) {
    ...
    server.delete('/:id/cronograma/dependencias/:depId', {
        schema: {
    ...
    server.post('/', { schema: { body: createContratoSchema } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        try {
            const created = await createContrato(tenantId, request.body);
            return reply.send(created);
        }
        catch (e) {
            if (e?.code === 'P2002') {
    ...
    server.put('/:id', { schema: { params: z.object({ id: z.coerce.number().int().positive() }), body: updateContratoSchema } }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { id } = request.params;
        try {
            const updated = await updateContrato(tenantId, id, request.body);
            return reply.send(updated);
        }
        catch (e) {
            if (e?.code === 'P2002') {

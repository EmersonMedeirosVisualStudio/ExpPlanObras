...
function computeVigencias(input) {
    ...
    const vigenciaInicial = input.vigenciaInicial ||
        (base && prazoDias != null ? addDays(dateOnly(base), prazoDias) : null);
    const vigenciaAtual = input.vigenciaAtual ||
...
function computeStatusEAlertas(row) {
    const issues = [];
    ...
    if (!dataOS && !dataAss)
        issues.push('Falta data de OS ou assinatura');
    if (!prazo || prazo <= 0)
        issues.push('Falta prazo (dias)');
    if (!row.empresaParceiraNome)
        issues.push('Empresa parceira não vinculada');
    if (temAditivoAberto)
        issues.push('Aditivo em aberto');
    const vti = toNumberOrNull(row.valorTotalInicial);
    if (vti == null)
        issues.push('Valor inicial não informado');
    if (valorTotalAtual <= 0)
        issues.push(valorTotalAtual === 0 ? 'Valor total do contrato é 0' : 'Valor total do contrato é negativo');
    const now = new Date();
    const statusManual = String(row.status || '').toUpperCase();
    let statusCalc = 'NAO_INICIADO';
    if (statusManual === 'CANCELADO')
        statusCalc = 'CANCELADO';
    else if (statusManual === 'RESCINDIDO')
        statusCalc = 'RESCINDIDO';
    else if (['CONCLUIDO', 'ENCERRADO', 'FINALIZADO'].includes(statusManual))
        statusCalc = 'CONCLUIDO';
    else if (['PARADO', 'PARALISADO'].includes(statusManual))
        statusCalc = 'PARADO';
    else if (['EM_EXECUCAO', 'ATIVO'].includes(statusManual))
        statusCalc = 'EM_EXECUCAO';
    else if (statusManual === 'PENDENTE')
        statusCalc = 'NAO_INICIADO';
    else if (!dataOS && valorExecutadoContrato <= 0)
        statusCalc = 'NAO_INICIADO';
    else
        statusCalc = 'EM_EXECUCAO';
    let alerta = 'OK';
...
async function createContratoEvento(tx, input) {
...
export async function ensureContratoPendente(tenantId) {
...
export async function listContratos(tenantId, opts) {
    return withRLS(tenantId, async (tx) => {
        const whereContrato = { tenantId };
        if (opts?.apenasPrincipais)
            whereContrato.contratoPrincipalId = null;
        ...
        if (tipoPapel)
            whereContrato.tipoPapel = tipoPapel;
      ...
      GROUP BY "contratoId"
    `;
        const valorPagoByContratoId = new Map();
        ...
        const valorExecutadoByContratoId = new Map();
        ...
        const aditivosAbertosByContratoId = new Map();
        for (const r of aditivosAbertos) {
            const id = Number(r.contratoId);
            if (!Number.isFinite(id))
                continue;
...
function normalizeSubStatus(v) {
    const s = String(v ?? '').trim().toUpperCase();
    if (s === 'PLANEJADO' || s === 'EM_EXECUCAO' || s === 'AGUARDANDO' || s === 'CONCLUIDO' || s === 'BLOQUEADO')
        return s;
    return 'EM_EXECUCAO';
}
function normalizeMedicaoStatus(v) {
    const s = String(v ?? '').trim().toUpperCase();
    if (s === 'PENDENTE' || s === 'APROVADO' || s === 'REJEITADO')
        return s;
    return 'PENDENTE';
}
export async function getSubcontratosResumo(tenantId, contratoPrincipalId) {
    return withRLS(tenantId, async (tx) => {
        const principal = await tx.contrato.findFirst({ where: { tenantId, id: contratoPrincipalId } }).catch(() => null);
        if (!principal)
            throw new Error('Contrato principal não encontrado');
        ...
        if (!valorPrincipal || valorPrincipal <= 0)
            alertas.push('Contrato principal sem valor total definido.');
        if (valorPrincipal > 0 && totalSub > valorPrincipal)
            alertas.push('Soma dos subcontratos ultrapassa o valor do contrato principal.');
        if (valorPrincipal > 0 && totalSub >= valorPrincipal * 0.9 && totalSub <= valorPrincipal)
            alertas.push('Soma dos subcontratos próxima do limite do contrato principal (>= 90%).');
        return {
            contratoPrincipal: {
                id: principal.id,
...
export async function listSubcontratos(tenantId, contratoPrincipalId) {
    return withRLS(tenantId, async (tx) => {
        const principal = await tx.contrato.findFirst({ where: { tenantId, id: contratoPrincipalId } }).catch(() => null);
        if (!principal)
            throw new Error('Contrato principal não encontrado');
        ...
        const ids = subs.map((s) => Number(s.id)).filter((n) => Number.isFinite(n));
        if (!ids.length)
            return [];
        const medAprovRows = await tx.$queryRaw `
      ...
      GROUP BY "contratoId"
    `;
        const pagRows = await tx.$queryRaw `
    ...
    `;
        ...
        for (const r of medAprovRows || [])
            medById.set(Number(r.contratoId), toNumberOrNull(r.medAprov) ?? 0);
        const pagById = new Map();
        for (const r of pagRows || [])
            pagById.set(Number(r.contratoId), toNumberOrNull(r.pago) ?? 0);
        ...
        return subs.map((s) => {
            ...
            if (fimPrincipal && fimSub && fimSub.getTime() > fimPrincipal.getTime())
                alertas.push('Subcontrato ultrapassa a vigência do contrato principal.');
            if (valor > 0 && medAprov > valor)
                alertas.push('Soma das medições aprovadas ultrapassa o valor do subcontrato.');
            if (pago > medAprov)
                alertas.push('Total pago ultrapassa total medido aprovado.');
            return {
                id,
...
export async function createSubcontrato(tenantId, contratoPrincipalId, input) {
    return withRLS(tenantId, async (tx) => {
        const principal = await tx.contrato.findFirst({ where: { tenantId, id: contratoPrincipalId } }).catch(() => null);
        if (!principal)
            throw new Error('Contrato principal não encontrado');
        ...
        if (!Number.isFinite(valorNovo) || valorNovo <= 0)
            throw new Error('Valor do subcontrato inválido');
...
export async function updateSubcontrato(tenantId, contratoPrincipalId, subcontratoId, input) {
    return withRLS(tenantId, async (tx) => {
        const principal = await tx.contrato.findFirst({ where: { tenantId, id: contratoPrincipalId } }).catch(() => null);
        if (!principal)
            throw new Error('Contrato principal não encontrado');
        ...
        if (!sub)
            throw new Error('Subcontrato não encontrado');
        const valorPrincipal = toNumberOrNull(principal.valorTotalAtual);
        ...
        if ((input.dataInicio != null || input.dataFim != null) && (!inicio || !fim))
            throw new Error('Datas inválidas');
...
export async function deleteSubcontrato(tenantId, contratoPrincipalId, subcontratoId) {
    return withRLS(tenantId, async (tx) => {
        const sub = await tx.contrato.findFirst({ where: { tenantId, id: subcontratoId, contratoPrincipalId } }).catch(() => null);
        if (!sub)
            throw new Error('Subcontrato não encontrado');
        ...
        if (mCount > 0 || pCount > 0)
            throw new Error('Não é possível excluir: existe medição ou pagamento vinculado ao subcontrato.');
...
export async function listContratoMedicoes(tenantId, contratoId) {
    return withRLS(tenantId, async (tx) => {
...
export async function createContratoMedicao(tenantId, contratoId, input) {
    return withRLS(tenantId, async (tx) => {
        ...
        if (valorContrato > 0 && total + amount > valorContrato)
            throw new Error('Soma das medições não pode ultrapassar o valor do contrato.');
...
export async function updateContratoMedicaoStatus(tenantId, contratoId, medicaoId, input) {
    return withRLS(tenantId, async (tx) => {
        ...
        if (status !== 'REJEITADO') {
            ...
            const total = toNumberOrNull(agg?._sum?.amount) ?? 0;
            if (valorContrato > 0 && total + (toNumberOrNull(med.amount) ?? 0) > valorContrato)
                throw new Error('Soma das medições não pode ultrapassar o valor do contrato.');
        }
...
export async function listContratoPagamentos(tenantId, contratoId) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId }, select: { id: true } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
...
export async function createContratoPagamento(tenantId, contratoId, input) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
        ...
        if (totalPago + amount > totalMedAprov)
            throw new Error('Não pode pagar mais que o total medido aprovado.');
        const medicaoId = input.medicaoId != null ? Number(input.medicaoId) : null;
        if (medicaoId != null) {
            const med = await tx.contratoMedicao.findFirst({ where: { tenantId, contratoId, id: medicaoId } }).catch(() => null);
            if (!med)
                throw new Error('Medição não encontrada');
            if (normalizeMedicaoStatus(med.status) !== 'APROVADO')
                throw new Error('Pagamento só pode vincular a medição aprovada.');
            ...
            if (pagoMed + amount > valorMed)
                throw new Error('Não pode pagar mais que o valor da medição vinculada.');
        }
...
export async function deleteContratoPagamento(tenantId, contratoId, pagamentoId) {
    return withRLS(tenantId, async (tx) => {
...
export async function listContratoProgramacaoFinanceira(tenantId, contratoId) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId }, select: { id: true } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
...
export async function createContratoProgramacaoFinanceira(tenantId, contratoId, input) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId }, select: { id: true } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
        ...
        if (!Number.isFinite(valorPrevisto) || valorPrevisto <= 0)
            throw new Error('Valor previsto inválido');
        try {
            ...
            if (msg.toLowerCase().includes('unique') || msg.toLowerCase().includes('duplicate'))
                throw new Error('Já existe programação cadastrada para esta competência.');
            throw e;
        }
    });
}
export async function updateContratoProgramacaoFinanceira(tenantId, contratoId, itemId, input) {
    return withRLS(tenantId, async (tx) => {
...
export async function deleteContratoProgramacaoFinanceira(tenantId, contratoId, itemId) {
    return withRLS(tenantId, async (tx) => {
        const row = await tx.contratoProgramacaoFinanceira.findFirst({ where: { tenantId, contratoId, id: itemId } }).catch(() => null);
        if (!row)
            throw new Error('Programação não encontrada');
...
export async function getContratoById(tenantId, id) {
    return withRLS(tenantId, async (tx) => {
        ...
        if (!contrato)
            return null;
        const obraIds = (contrato.obras || []).map((o) => Number(o.id)).filter((n) => Number.isFinite(n));
        ...
        const abertos = await tx.$queryRaw `
...
export async function listContratoAditivos(tenantId, contratoId) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId }, select: { id: true } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
...
export async function createContratoAditivo(tenantId, contratoId, input) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
        ...
        if (!numeroAditivo)
            throw new Error('numeroAditivo é obrigatório');
        const tipo = String(input.tipo || 'PRAZO').trim().toUpperCase();
        if (tipo === 'AMBOS')
            throw new Error('Tipo AMBOS é legado. Use: Prazo, Valor ou Reprogramação.');
        const assinatura = input.dataAssinatura ? parseDateOnly(input.dataAssinatura) : null;
        if (!assinatura)
            throw new Error('dataAssinatura é obrigatória');
        const hoje = dateOnly(new Date());
        if (dateOnly(assinatura).getTime() > hoje.getTime())
            throw new Error('dataAssinatura não pode ser no futuro');
        if (contrato.dataAssinatura) {
            const inicio = dateOnly(new Date(contrato.dataAssinatura));
            if (dateOnly(assinatura).getTime() < inicio.getTime())
                throw new Error('dataAssinatura do aditivo deve ser maior ou igual à data de assinatura do contrato');
        }
        if (contrato.vigenciaAtual) {
            const fim = dateOnly(new Date(contrato.vigenciaAtual));
            if (dateOnly(assinatura).getTime() > fim.getTime())
                throw new Error('dataAssinatura do aditivo não pode ultrapassar a vigência atual do contrato');
        }
        ...
        const baseForPrazo = contrato.vigenciaAtual ? new Date(contrato.vigenciaAtual) : contrato.dataOS ? new Date(contrato.dataOS) : contrato.dataAssinatura ? new Date(contrato.dataAssinatura) : null;
        ...
        if (tipo === 'PRAZO' && (!prazoAdicionadoDias || prazoAdicionadoDias <= 0))
            throw new Error('Prazo: informe a data fim da vigência ou o prazo adicionado (dias) > 0');
        if (tipo === 'VALOR') {
            const atual = toNumberOrNull(contrato.valorTotalAtual) ?? 0;
            if (valorTotalAdicionado == null || !Number.isFinite(valorTotalAdicionado) || valorTotalAdicionado <= 0)
                throw new Error('Informe o valor total do contrato após o aditivo (deve ser > 0)');
            if (valorTotalAdicionado === atual)
                throw new Error('O valor total (após aditivo) deve ser diferente do valor atual do contrato');
        }
...
export async function updateContratoAditivo(tenantId, contratoId, aditivoId, input) {
    return withRLS(tenantId, async (tx) => {
        ...
        if (String(current.status).toUpperCase() !== 'RASCUNHO')
            throw new Error('Somente aditivos em rascunho podem ser alterados');
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
        ...
        if (!allowed.has(tipo))
            throw new Error('Tipo de aditivo inválido');
        const dataInicioVigencia = input.dataInicioVigencia != null ? parseDateOnly(input.dataInicioVigencia) : current.dataInicioVigencia ? new Date(current.dataInicioVigencia) : null;
        const dataFimVigencia = input.dataFimVigencia != null ? parseDateOnly(input.dataFimVigencia) : current.dataFimVigencia ? new Date(current.dataFimVigencia) : null;
        const baseForPrazo = contrato.vigenciaAtual ? new Date(contrato.vigenciaAtual) : contrato.dataOS ? new Date(contrato.dataOS) : contrato.dataAssinatura ? new Date(contrato.dataAssinatura) : null;
        let prazoAdicionadoDias = input.prazoAdicionadoDias != null ? Math.trunc(Number(input.prazoAdicionadoDias)) : current.prazoAdicionadoDias != null ? Math.trunc(Number(current.prazoAdicionadoDias)) : null;
        ...
        const alterouPlanilhaFinal = tipo === 'VALOR' || tipo === 'AMBOS' ? true : input.alterouPlanilha != null ? Boolean(input.alterouPlanilha) : Boolean(current.alterouPlanilha);
        if (tipo === 'PRAZO' && (!prazoAdicionadoDias || prazoAdicionadoDias <= 0))
            throw new Error('Prazo: informe a data fim da vigência ou o prazo adicionado (dias) > 0');
        ...
        const updated = await tx.contratoAditivo.update({
            ...
            data: {
                ...
                prazoAdicionadoDias: input.prazoAdicionadoDias != null || input.dataFimVigencia != null
                    ? tipo === 'PRAZO' || tipo === 'AMBOS'
                        ? prazoAdicionadoDias
                        : null
                    : undefined,
                valorTotalAdicionado: input.valorTotalAdicionado != null || input.tipo != null
...
export async function cancelarContratoAditivo(tenantId, contratoId, aditivoId) {
    return withRLS(tenantId, async (tx) => {
        const current = await tx.contratoAditivo.findFirst({ where: { tenantId, contratoId, id: aditivoId } }).catch(() => null);
        if (!current)
            throw new Error('Aditivo não encontrado');
        if (String(current.status).toUpperCase() === 'APROVADO')
            throw new Error('Aditivo aprovado não pode ser cancelado');
...
export async function aprovarContratoAditivo(tenantId, contratoId, aditivoId) {
    return withRLS(tenantId, async (tx) => {
        const ad = await tx.contratoAditivo.findFirst({ where: { tenantId, contratoId, id: aditivoId } }).catch(() => null);
        if (!ad)
            throw new Error('Aditivo não encontrado');
        if (String(ad.status).toUpperCase() !== 'RASCUNHO')
            throw new Error('Aditivo já foi processado');
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
        const tipo = String(ad.tipo || 'PRAZO').toUpperCase();
        const planilhaVersaoAtual = contrato.planilhaVersao != null ? Math.trunc(Number(contrato.planilhaVersao)) : 1;
        const alterouPlanilhaFinal = Boolean(ad.alterouPlanilha) || tipo === 'VALOR' || tipo === 'AMBOS';
        ...
        let novaVigenciaAtual = vigAtual;
        const aplicaPrazo = tipo === 'PRAZO' || tipo === 'AMBOS';
        const aplicaValor = tipo === 'VALOR' || tipo === 'AMBOS';
        if (aplicaPrazo) {
            if (!prazoAdd || prazoAdd <= 0)
                throw new Error('prazoAdicionadoDias deve ser > 0');
            if (vigAtual)
                novaVigenciaAtual = addDays(dateOnly(vigAtual), prazoAdd);
        ...
        if (aplicaValor) {
            ...
            if (novoTotal == null || !Number.isFinite(novoTotal) || novoTotal <= 0)
                throw new Error('valorTotalAdicionado (novo total após aditivo) deve ser > 0');
        }
...
export async function getContratoConsolidado(tenantId, contratoId) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
        const obraIds = await tx.obra.findMany({ where: { tenantId, contratoId }, select: { id: true } });
        const ids = obraIds.map((o) => o.id);
...
export async function listContratoEventos(tenantId, contratoId, input) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId }, select: { id: true } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
        ...
        if (desde)
            createdAtFilter.gte = desde;
        if (ate)
            createdAtFilter.lte = ate;
        const hasCreatedAtFilter = Object.keys(createdAtFilter).length > 0;
        const where = { tenantId, contratoId };
        if (tiposOrigem.length)
            where.tipoOrigem = { in: tiposOrigem };
        if (!incluirObservacoes)
            where.tipoEvento = { not: 'OBSERVACAO' };
        ...
        const anexos = eventoIds.length
            ...
            : [];
        const anexosByEventoId = new Map();
        ...
        const contratoEventos = eventos.map((e) => ({
            id: e.id,
        ...
        const actions = obras.map((o) => `OBRA:${o.id}`);
        if (!actions.length)
            return contratoEventos;
        const obraByAction = new Map();
        for (const o of obras)
            obraByAction.set(`OBRA:${o.id}`, Number(o.id));
        const whereHist = { tenantId, action: { in: actions } };
        if (hasCreatedAtFilter)
            whereHist.createdAt = createdAtFilter;
        if (texto)
            whereHist.message = { contains: texto, mode: 'insensitive' };
        const hist = await tx.tenantHistoryEntry.findMany({
            where: whereHist,
            orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
            take: Math.min(500, limit * 10),
        });
        const obraEventos = hist.map((h) => ({
            id: -Number(h.id),
            tipoOrigem: 'OBRA',
...
export async function createContratoObservacao(tenantId, contratoId, input) {
    return withRLS(tenantId, async (tx) => {
...
export async function addContratoEventoAnexo(tenantId, contratoId, eventoId, input) {
    return withRLS(tenantId, async (tx) => {
        ...
        if (!nomeArquivo)
            throw new Error('nomeArquivo é obrigatório');
        if (!mimeType)
...
export async function downloadContratoEventoAnexo(tenantId, contratoId, eventoId, anexoId) {
    return withRLS(tenantId, async (tx) => {
...
export async function getContratosDashboard(tenantId, input) {
    return withRLS(tenantId, async (tx) => {
        const statusRaw = input?.status ? String(input.status).trim().toUpperCase() : null;
        const statusIn = statusRaw === 'NAO_INICIADO'
        ...
        const whereContrato = { tenantId, contratoPrincipalId: null };
        if (statusIn && statusIn.length)
            whereContrato.status = { in: statusIn };
        if (tipoPapel)
            whereContrato.tipoPapel = tipoPapel;
        if (tipoContratante)
            whereContrato.tipoContratante = tipoContratante;
        const now = new Date();
        ...
        const [valorExecutadoAgg, valorPagoAgg] = await Promise.all([
            tx.medicao.aggregate({
                ...
                where: {
                    obra: {
                        tenantId,
                        contrato: (statusIn && statusIn.length) || tipoPapel || tipoContratante
                            ? {
        ...
        function normalizeStatusForDashboard(row) {
            const st = String(row?.status || '').toUpperCase();
            if (st === 'CANCELADO')
                return 'CANCELADO';
            if (st === 'RESCINDIDO')
                return 'RESCINDIDO';
            if (['CONCLUIDO', 'ENCERRADO', 'FINALIZADO'].includes(st))
                return 'CONCLUIDO';
            if (['PARADO', 'PARALISADO'].includes(st))
                return 'PARADO';
            if (['EM_EXECUCAO', 'ATIVO'].includes(st))
                return 'EM_EXECUCAO';
            if (st === 'PENDENTE')
                return 'NAO_INICIADO';
            const dataOS = row?.dataOS ? new Date(row.dataOS) : null;
        ...
        const aditivosPorStatus = {};
        for (const a of aditivosAgg) {
            aditivosPorStatus[String(a.status || '').toUpperCase()] = Number(a?._count?._all ?? 0);
        }
        ...
        const prazoCriticoRows = contratos
            ...
            if (closedStatuses.has(st))
                return false;
            return !!c.vigenciaAtual;
        })
            .map((c) => {
            const vig = c.vigenciaAtual;
        ...
        for (const c of contratos) {
            const t = String(c.tipoContratante || 'PRIVADO').toUpperCase();
            tipoCount.set(t, (tipoCount.get(t) ?? 0) + 1);
        }
        for (const [tipo, quantidade] of tipoCount.entries())
            porTipo.push({ tipo, quantidade });
        ...
        const executadoMesRows = statusIn && statusIn.length
            ? await tx.$queryRaw `
        ...
        `
          ...
          ORDER BY 1 ASC
        `;
        const contratadoMesRows = statusIn && statusIn.length
            ? await tx.$queryRaw `
...
function nextMonthStartUTC(y, m) {
...
function buildMonthRange(startYm, endYm) {
    ...
    if (endNext.getTime() <= start.getTime())
        return null;
    const months = [];
    let cy = a.y;
    let cm = a.m;
...
export async function getContratosFaturamento(tenantId, input) {
    return withRLS(tenantId, async (tx) => {
        ...
        if (contratoId && Number.isFinite(contratoId)) {
            ...
            const rows = await tx.contrato.findMany({
                where: {
                    ...
                    empresaParceiraNome: { contains: empresaQ, mode: 'insensitive' },
                },
                select: { id: true },
                take: 200,
            });
        ...
        const receitaRows = contratoIds == null
            ? await tx.$queryRaw `
          ...
          `
            ...
            `
                : [];
        const custoRows = contratoIds == null
            ? await tx.$queryRaw `
          ...
          `
            ...
            `
                : [];
        const subPagRows = contratoId && Number.isFinite(contratoId)
            ? await tx.$queryRaw `
          SELECT to_char(date_trunc('month', p."date"), 'YYYY-MM') AS "mes", COALESCE(SUM(p."amount"), 0) AS "total"
        ...
        `
          ...
          ORDER BY 1 ASC
        `;
        const receitaByMes = new Map();
        for (const r of receitaRows || []) {
            const mes = String(r.mes || '');
            const v = r.total;
            const n = typeof v === 'number' ? v : v ? Number(v) : 0;
            if (mes)
                receitaByMes.set(mes, Number.isFinite(n) ? n : 0);
        }
        const custoByMes = new Map();
        for (const r of custoRows || []) {
            const mes = String(r.mes || '');
            const v = r.total;
            const n = typeof v === 'number' ? v : v ? Number(v) : 0;
            if (mes)
                custoByMes.set(mes, Number.isFinite(n) ? n : 0);
        }
        const subPagByMes = new Map();
        for (const r of subPagRows || []) {
            const mes = String(r.mes || '');
            const v = r.total;
            const n = typeof v === 'number' ? v : v ? Number(v) : 0;
            if (mes)
                subPagByMes.set(mes, Number.isFinite(n) ? n : 0);
        }
...
export async function createContrato(tenantId, input) {
    return withRLS(tenantId, async (tx) => {
        ...
        if (contratoPrincipalId != null) {
            if (!Number.isFinite(contratoPrincipalId) || contratoPrincipalId <= 0)
                throw new Error('Contrato vinculado inválido');
            if (tipoPapel !== 'CONTRATANTE')
                throw new Error('Contrato vinculado (subcontrato) deve ter papel "Somos contratantes".');
            ...
            if (!principal)
                throw new Error('Contrato vinculado não encontrado');
            if (principal.contratoPrincipalId != null)
                throw new Error('Contrato vinculado deve ser um contrato principal');
        }
        ...
        if (totalInicial == null || !Number.isFinite(totalInicial) || totalInicial <= 0)
...
export async function updateContrato(tenantId, id, input) {
    return withRLS(tenantId, async (tx) => {
        ...
        const tipoPapelNext = rawPapel === 'CONTRATANTE' ? 'CONTRATANTE' : rawPapel === 'CONTRATADO' ? 'CONTRATADO' : String(current.tipoPapel || 'CONTRATADO').trim().toUpperCase() === 'CONTRATANTE' ? 'CONTRATANTE' : 'CONTRATADO';
        const contratoPrincipalIdNext = input.contratoPrincipalId !== undefined
        ...
        if (contratoPrincipalIdNext != null) {
            if (!Number.isFinite(contratoPrincipalIdNext) || contratoPrincipalIdNext <= 0)
                throw new Error('Contrato vinculado inválido');
            if (contratoPrincipalIdNext === id)
                throw new Error('Contrato vinculado inválido');
            if (tipoPapelNext !== 'CONTRATANTE')
                throw new Error('Contrato vinculado (subcontrato) deve ter papel "Somos contratantes".');
            ...
            if (principal.contratoPrincipalId != null)
                throw new Error('Contrato vinculado deve ser um contrato principal');
            if (subCount > 0)
                throw new Error('Não é possível vincular este contrato: ele já possui subcontratos.');
        }
        ...
        if (input.valorTotalInicial != null) {
            if (totalInicialNext == null || !Number.isFinite(totalInicialNext) || totalInicialNext <= 0)
                throw new Error('Valor total do contrato deve ser maior que zero');
        }
        const updated = await tx.contrato.update({
            where: { id },
...
export async function listContratoServicos(tenantId, contratoId) {
    return withRLS(tenantId, async (tx) => {
...
export async function createContratoServico(tenantId, contratoId, input) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId }, select: { id: true } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
        ...
        const valorTotal = quantidade != null && Number.isFinite(quantidade) && valorUnitario != null && Number.isFinite(valorUnitario)
...
export async function seedCronogramaFromServicos(tenantId, contratoId, input) {
    return withRLS(tenantId, async (tx) => {
        ...
        const existingSet = new Set(existing.map((e) => Number(e.servicoId)));
        const now = new Date();
...
export async function getContratoCronograma(tenantId, contratoId) {
    return withRLS(tenantId, async (tx) => {
        const contrato = await tx.contrato.findFirst({ where: { tenantId, id: contratoId }, select: { id: true } }).catch(() => null);
        if (!contrato)
            throw new Error('Contrato não encontrado');
...
export async function updateCronogramaItemDatas(tenantId, contratoId, itemId, input) {
    return withRLS(tenantId, async (tx) => {
...
export async function createCronogramaDependencia(tenantId, contratoId, input) {
    return withRLS(tenantId, async (tx) => {
        ...
        if (!origem || !destino)
            throw new Error('Dependência inválida');
        if (input.origemItemId === input.destinoItemId)
...
export async function deleteCronogramaDependencia(tenantId, contratoId, depId) {
    return withRLS(tenantId, async (tx) => {

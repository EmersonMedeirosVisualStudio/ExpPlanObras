...
function extractLatLngFromGoogleMapsLink(link) {
    ...
    if (queryLatLonMatch)
        return { lat: queryLatLonMatch[1], lon: queryLatLonMatch[2] };
    return null;
}
...
export default async function geoRoutes(server) {
    ...
    server.post('/cep/resolve', {
        ...
        const res = await fetchJson(url);
        if (!res.ok)
            return reply.code(400).send({ message: 'Falha ao consultar CEP' });
        if (res.data?.erro)
            return reply.code(404).send({ message: 'CEP não encontrado' });
        return reply.send({
            source: 'CEP',
    ...
    server.post('/geocode', {
        ...
        const url = `https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=0&limit=1&q=${encodeURIComponent(q)}`;
        const res = await fetchJson(url);
        if (!res.ok)
            return reply.code(400).send({ message: 'Falha ao consultar geocodificação' });
        const item = Array.isArray(res.data) ? res.data[0] : null;
        if (!item)
            return reply.code(404).send({ message: 'Localização não encontrada' });
        return reply.send({ latitude: String(item?.lat || ''), longitude: String(item?.lon || '') });
    });
    server.get('/ibge/municipios', {
        schema: {
        ...
        const res = await fetchJson(url);
        if (!res.ok)
            return reply.code(400).send({ message: 'Falha ao consultar municípios' });
    ...
    server.get('/ibge/search-city', {
        ...
        const res = await fetchJson(url);
        if (!res.ok)
            return reply.code(400).send({ message: 'Falha ao consultar cidade' });
        const list = Array.isArray(res.data) ? res.data : [];
        const mapped = list
            .map((m) => ({
            ...
            .filter((x) => x.city.length > 0 && x.uf.length === 2);
        return reply.send(mapped);
    });
    server.get('/cep/search', {
        schema: {
        ...
        const { uf, city, street } = request.query;
        const UF = String(uf || '').toUpperCase();
        const c = String(city || '').trim();
        const s = String(street || '').trim();
        const url = `https://viacep.com.br/ws/${encodeURIComponent(UF)}/${encodeURIComponent(c)}/${encodeURIComponent(s)}/json/`;
        const res = await fetchJson(url);
        if (!res.ok)
            return reply.code(400).send({ message: 'Falha ao consultar CEP' });
        const list = Array.isArray(res.data) ? res.data : [];
        const ceps = list
            .map((i) => String(i?.cep || '').replace(/\D+/g, ''))
            .filter((v) => v.length === 8);

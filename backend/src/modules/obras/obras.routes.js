...
import { createObra, getObras, getObraById, updateObra, deleteObra, getOrcamento, updateOrcamento, addCusto, removeCusto, getEnderecoObra, upsertEnderecoObra, listEnderecosObra, createEnderecoObra, updateEnderecoObraById, deleteEnderecoObraById, ensurePlanilhaContratadaMinima, getPlanilhaContratadaResumo, listPlanilhaContratadaItens, addPlanilhaContratadaItem, getObrasResumoFinanceiro } from './obras.service.js';
import { authenticate } from '../../utils/authenticate.js';
export default async function obraRoutes(server) {
    ...
    function normalizeUfFromNominatim(candidate, isoCandidate, stateNameCandidate) {
    ...
    server.post('/enderecos/preview/link', {
        schema: {
            body: z.object({
                link: z.string().min(8),
            }),
        ...
        try {
            ...
            if (!coords)
                return reply.code(400).send({ message: 'Não foi possível extrair latitude/longitude do link' });
    ...
    server.post('/enderecos/preview/cep', {
        schema: {
            body: z.object({
                cep: z.string().min(8),
            }),
    ...
    server.post('/enderecos/preview/buscar-cep', {
        ...
        try {
            ...
            if (!q)
                return reply.code(400).send({ message: 'Informe rua, cidade e UF para buscar o CEP' });
            const res = await searchGeocode(q);
            if (!res?.cep)
                return reply.code(400).send({ message: 'Não foi possível localizar o CEP para este endereço' });
    ...
    server.put('/:id/endereco', {
        ...
        const { id } = request.params;
        const body = request.body;
        const origem = String(body.origem || 'MANUAL').toUpperCase();
        try {
            if (origem === 'CEP') {
                const base = await lookupCep(String(body.cep || ''));
                if (!base)
                    return reply.code(400).send({ message: 'CEP inválido' });
                const saved = await upsertEnderecoObra(id, tenantId, {
                    ...base,
    ...
    server.post('/:id/enderecos', { schema: { params: z.object({ id: z.coerce.number().int().positive() }), body: enderecoBodySchema } }, async (request, reply) => {
        ...
        try {
            if (origem === 'CEP') {
                ...
                if (!base)
                    return reply.code(400).send({ message: 'CEP inválido' });
                const saved = await createEnderecoObra(id, tenantId, {
                    ...base,
                    numero: body.numero ?? null,
    ...
    server.put('/:id/enderecos/:enderecoId', {
        schema: {
        ...
        const { tenantId } = request.user;
        const scope = request.user?.abrangencia;
        const { id, enderecoId } = request.params;
        const body = request.body;
        const origem = String(body.origem || 'MANUAL').toUpperCase();
        try {
            if (origem === 'CEP') {
                ...
                if (!base)
                    return reply.code(400).send({ message: 'CEP inválido' });
                const saved = await updateEnderecoObraById(id, enderecoId, tenantId, {
                    ...base,
                    numero: body.numero ?? null,
                    nomeEndereco: body.nomeEndereco ?? null,
                    principal: body.principal ?? null,
    ...
    server.post('/:id/planilha/itens', {
        ...
        const { id } = request.params;
        try {
            const result = await addPlanilhaContratadaItem(id, tenantId, request.body, scope);
            return reply.send(result);
        }
        catch (e) {
    ...
    server.post('/:id/custos', {
        ...
        const { tenantId } = request.user;
        const { id } = request.params;
        const data = await addCusto(id, request.body, tenantId);
        return reply.code(201).send(data);
    });
    server.delete('/:id/custos/:custoId', {
        schema: {
            params: z.object({
                id: z.coerce.number().int(),
        ...
        const { tenantId } = request.user;
        const { id, custoId } = request.params;
    ...
    server.post('/', {
        ...
        const { tenantId } = request.user;
        const obra = await createObra(request.body, tenantId);
    ...
    server.put('/:id', {
        ...
        const { id } = request.params;
        try {
            const obra = await updateObra(id, request.body, tenantId, scope);
            return reply.send(obra);
        }
        catch (error) {
            return reply.code(404).send({ message: 'Obra not found' });

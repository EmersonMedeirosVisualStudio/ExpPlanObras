...
async function requireAdminOrEncarregado(request, reply) {
...
function mapProviderStatusToLocal(status) {
    ...
    if (['SIGNED', 'ASSINADA', 'COMPLETED', 'CONCLUIDA', 'DONE'].includes(s))
        return 'ASSINADA';
    if (['REJECTED', 'REJEITADA'].includes(s))
        return 'REJEITADA';
    if (['CANCELED', 'CANCELADA'].includes(s))
        return 'CANCELADA';
    if (['EXPIRED', 'EXPIRADA'].includes(s))
        return 'EXPIRADA';
    if (['ERROR', 'ERRO', 'FAILED', 'FALHOU'].includes(s))
        return 'ERRO';
    if (['PARTIAL', 'PARCIAL'].includes(s))
        return 'PARCIAL';
    if (['SENT', 'ENVIADA'].includes(s))
        return 'AGUARDANDO_ASSINATURA';
    if (s)
        return 'AGUARDANDO_ASSINATURA';
    return 'AGUARDANDO_ASSINATURA';
}
export default async function documentosQualificadosRoutes(server) {
    ...
    server.post('/provedores', {
        ...
        try {
            if (body.clientSecret)
                clientSecretEnc = encryptSecret(String(body.clientSecret));
            if (body.apiKey)
                apiKeyEnc = encryptSecret(String(body.apiKey));
        }
        catch (e) {
    ...
    server.post('/solicitacoes', {
        ...
        if (!versaoId) {
            ...
            if (!v || v.tenantId !== ctx.tenantId || v.documentoId !== doc.id)
                return fail(reply, 404, 'Versão do documento inválida');
        }
        const provedor = await prisma.documentoAssinaturaProvedor.findUnique({ where: { id: body.provedorId } }).catch(() => null);
        if (!provedor || provedor.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Provedor não encontrado');
        const expiraEm = body.expiraEm ? new Date(String(body.expiraEm)) : null;
        if (expiraEm && Number.isNaN(expiraEm.getTime()))
            return fail(reply, 400, 'expiraEm inválido');
    ...
    server.get('/solicitacoes', async (request, reply) => {
        ...
        if (q.status)
            where.statusSolicitacao = String(q.status).toUpperCase();
    ...
    server.get('/solicitacoes/:id', async (request, reply) => {
        ...
        if (!row || row.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Solicitação não encontrada');
        return ok(reply, {
            ...row,
    ...
    server.post('/solicitacoes/:id/enviar', async (request, reply) => {
        ...
        if (s.statusSolicitacao !== 'RASCUNHO' && s.statusSolicitacao !== 'ERRO')
            return fail(reply, 409, 'Solicitação não pode ser enviada neste status');
        if (!s.callbackToken)
            return fail(reply, 400, 'Solicitação sem callbackToken');
        const providerImpl = getQualifiedSignatureProvider(s.provedor.codigo);
        if (!providerImpl)
            return fail(reply, 501, `Provedor não suportado em código: ${s.provedor.codigo}`);
        ...
        if (!docRes || !docRes.ok)
            return fail(reply, 502, 'Falha ao baixar o documento original');
        ...
        if (!callbackUrl.startsWith('http'))
            return fail(reply, 501, 'PUBLIC_API_URL não configurado para receber callback do provedor');
        const created = await providerImpl.createEnvelope({
            ...
            config: s.provedor.configuracaoJson || null,
        });
    ...
    server.post('/solicitacoes/:id/sincronizar', async (request, reply) => {
        ...
        if (!s || s.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Solicitação não encontrada');
        if (!s.providerEnvelopeId)
            return fail(reply, 400, 'Solicitação sem envelopeId');
        const providerImpl = getQualifiedSignatureProvider(s.provedor.codigo);
        if (!providerImpl)
            return fail(reply, 501, `Provedor não suportado em código: ${s.provedor.codigo}`);
        const status = await providerImpl.getEnvelopeStatus({ tenantId: s.tenantId, envelopeId: s.providerEnvelopeId, config: s.provedor.configuracaoJson || null });
        const localStatus = mapProviderStatusToLocal(status.status);
        let concluded = false;
        let signedArtifactId = null;
        const updated = await prisma.$transaction(async (tx) => {
            ...
            if (localStatus === 'ASSINADA') {
                const dl = await providerImpl.downloadSignedDocument({ tenantId: s.tenantId, envelopeId: s.providerEnvelopeId, config: s.provedor.configuracaoJson || null });
    ...
    server.post('/solicitacoes/:id/verificar', async (request, reply) => {
        ...
        if (!art || !art.data)
            return fail(reply, 404, 'PDF assinado não encontrado');
        const providerImpl = getQualifiedSignatureProvider(s.provedor.codigo);
        let valid = null;
        try {
            if (providerImpl?.verifyDocument) {
                const res = await providerImpl.verifyDocument({ tenantId: ctx.tenantId, buffer: Buffer.from(art.data), config: s.provedor.configuracaoJson || null });
    ...
    server.post('/callback/:provider', async (request, reply) => {
        const providerCode = String(request.params?.provider || '').trim().toUpperCase();

...
export async function exportTenantBackup(tenantId) {
...
export async function restoreTenantBackup(tenantId, backup) {
    if (!backup || typeof backup !== 'object')
        throw new Error('Backup inválido');
    if (backup?.tenant?.id !== tenantId)
        throw new Error('Backup não corresponde ao tenant');
    return prisma.$transaction(async (tx) => {
        ...
        await tx.tenantHistoryAttachment.deleteMany({ where: { entry: { tenantId } } });
        ...
        for (const o of backup.obras || []) {
            const contratoIdOriginal = o.contratoId != null ? Number(o.contratoId) : null;

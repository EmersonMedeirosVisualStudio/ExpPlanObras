...
export async function calcularScoreQualidadePorAtivo(args) {
    ...
    if (!regras.length)
        return { score: 100, status: 'SAUDAVEL' };
    ...
    for (const e of execs) {
        if (latestByRule.has(e.regraId))
            continue;
        const st = String(e.statusExecucao || '').toUpperCase();
        if (st === 'OK' || st === 'ALERTA' || st === 'FALHA' || st === 'ERRO')
            latestByRule.set(e.regraId, st);
    }
    let score = 100;

...
async function requireGovernancaAdmin(request, reply) {
...
export default async function governancaDadosRoutes(server) {
    ...
    server.post('/pii-scans', {
        ...
        if (!res.ok)
            return fail(reply, 400, res.reason || 'Falha no scan');
    ...
    server.get('/pii-scans', async (request, reply) => {
        ...
        if (q.status)
            where.statusScan = String(q.status).toUpperCase();
    ...
    server.get('/pii-scans/:id/resultados', async (request, reply) => {
        ...
        if (!scan || scan.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Scan não encontrado');
    ...
    server.get('/classificacao/sugestoes', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusSugestao = String(q.status).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.post('/classificacao/sugestoes/:id/aceitar', async (request, reply) => {
        ...
        if (!res.ok)
            return fail(reply, 400, res.reason || 'Falha');
        return ok(reply, { ok: true }, { message: 'Sugestão aceita' });
    });
    server.post('/classificacao/sugestoes/:id/rejeitar', {
    ...
    server.get('/ativos', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.tipo)
            where.tipoAtivo = String(q.tipo).toUpperCase();
        if (q.dominio) {
            ...
            if (!dom)
                return ok(reply, [], { meta: { pagina: q.pagina, limite: q.limite, total: 0 } });
    ...
    server.get('/ativos/:id', async (request, reply) => {
        ...
        if (!ativo || ativo.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Ativo não encontrado');
    ...
    server.post('/lineage', {
        ...
        if (!origem || origem.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Ativo origem inválido');
        if (!destino || destino.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Ativo destino inválido');
    ...
    server.get('/glossario', async (request, reply) => {
        ...
        if (q.termo)
            where.termo = { contains: q.termo, mode: 'insensitive' };
        if (q.dominio) {
            const dom = await prisma.governancaDadoDominio.findFirst({ where: { tenantId: ctx.tenantId, codigoDominio: String(q.dominio).toUpperCase() } });
            if (!dom)
                return ok(reply, [], { meta: { pagina: q.pagina, limite: q.limite, total: 0 } });
    ...
    server.get('/qualidade/regras', async (request, reply) => {
        ...
        const q = z
            ...
            .parse(request.query || {});
        const where = { tenantId: ctx.tenantId };
        if (typeof q.ativoId === 'number')
            where.ativoId = q.ativoId;
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.put('/qualidade/regras/:id', {
        schema: {
            params: z.object({ id: z.coerce.number().int() }),
            body: z.object({
                caminhoCampo: z.string().optional().nullable(),
        ...
        if (!regra || regra.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Regra não encontrada');
    ...
    server.post('/qualidade/regras/:id/executar', async (request, reply) => {
        ...
        let statusExecucao = 'OK';
        let totalRegistros = null;
        let totalInconsistencias = null;
        let valorApurado = null;
        let mensagemResultado = null;
        try {
            if (tipo === 'FRESHNESS') {
                const cfg = regra.configuracaoJson;
                ...
                if (!objeto || !prisma[objeto])
                    throw new Error('OBJETO_NAO_SUPORTADO');
                const row = await prisma[objeto].findFirst({ orderBy: { [referenceField]: 'desc' }, select: { [referenceField]: true } });
                const dt = row?.[referenceField] ? new Date(row[referenceField]) : null;
                if (!dt || Number.isNaN(dt.getTime()))
                    throw new Error('SEM_REFERENCIA');
                const delayMin = Math.floor((Date.now() - dt.getTime()) / 60000);
                valorApurado = delayMin;
                if (delayMin > maxDelayMinutes)
                    statusExecucao = 'FALHA';
                else if (delayMin > Math.floor(maxDelayMinutes * 0.7))
                    statusExecucao = 'ALERTA';
                ...
                if (!objeto || !prisma[objeto])
                    throw new Error('OBJETO_NAO_SUPORTADO');
                const total = await prisma[objeto].count();
                const missing = await prisma[objeto].count({ where: { OR: [{ [field]: null }, { [field]: '' }] } });
                ...
                if (!field || min === null || max === null)
                    throw new Error('CONFIG_INVALIDA');
                if (!objeto || !prisma[objeto])
                    throw new Error('OBJETO_NAO_SUPORTADO');
                const total = await prisma[objeto].count();
                const out = await prisma[objeto].count({ where: { OR: [{ [field]: { lt: min } }, { [field]: { gt: max } }] } });
                totalRegistros = total;
        ...
        if (statusExecucao === 'FALHA' || statusExecucao === 'ALERTA') {
            ...
            if (existing) {
                await prisma.governancaDadoQualidadeIssue.update({
                    ...
                    data: { ultimaOcorrenciaEm: now, severidade: regra.severidade, metadataJson: { lastExecId: exec.id, statusExecucao } },
                });
            }
            else {
    ...
    server.get('/qualidade/issues', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusIssue = String(q.status).toUpperCase();
        if (q.severidade)
            where.severidade = String(q.severidade).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([

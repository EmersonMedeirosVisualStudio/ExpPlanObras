...
import { detectByFieldName, detectByValue } from './pii-detectors.js';
function mapToMask(type) {
    if (type === 'CPF')
        return 'CPF';
    if (type === 'CNPJ')
        return 'CNPJ';
...
function suggestClassification(type) {
    if (type === 'CPF' || type === 'CNPJ')
        return 'RESTRITO';
    return 'SENSIVEL';
}
function maxConfidence(a, b) {
    const rank = (x) => (x === 'CONFIRMADO' ? 3 : x === 'PROVAVEL' ? 2 : x === 'SUSPEITO' ? 1 : 0);
    return rank(a) >= rank(b) ? a : b;
}
export async function executarScanPiiAmostral(args) {
    const ativo = await prisma.governancaDadoAtivo.findUnique({ where: { id: args.ativoId } }).catch(() => null);
    if (!ativo || ativo.tenantId !== args.tenantId)
        return { ok: false, reason: 'ATIVO_INVALIDO' };
    ...
    const model = modelName && prisma[modelName] ? prisma[modelName] : null;
    if (!model) {
        await prisma.governancaPiiScan.update({
            where: { id: scan.id },
            data: { statusScan: 'ERRO', finalizadoEm: new Date(), resultadoResumoJson: { error: 'OBJETO_NAO_SUPORTADO' } },
        });
        return { ok: false, reason: 'OBJETO_NAO_SUPORTADO', scanId: scan.id };
    }
    const take = Math.min(Math.max(Number(args.sampleSize || 20), 1), 50);
    const rows = (await model.findMany({ where: { tenantId: args.tenantId }, take, orderBy: { id: 'desc' } }).catch(() => []));
    const results = [];
    for (const c of campos) {
        const fieldPath = String(c.caminhoCampo);
        const byName = detectByFieldName(fieldPath);
        let detectedType = byName ? byName.type : null;
        let confidence = byName ? byName.confidence : 'NAO_IDENTIFICADO';
        let detector = byName ? 'FIELD_NAME' : '';
        let maskedSample = null;
        for (const r of rows) {
            const v = r?.[fieldPath];
            ...
            maskedSample = String(applyMask(mask, v) ?? '');
            if (byValue.confidence === 'CONFIRMADO')
                break;
        }
        if (!detectedType || confidence === 'NAO_IDENTIFICADO')
            continue;
    ...
    for (const r of results) {
        ...
        const row = await prisma.governancaPiiScanResultado.create({
            data: {
                ...
                metadataJson: { fieldPath: r.fieldPath },
            },
        });
    ...
    await prisma.governancaPiiScan.update({
        ...
        data: {
            ...
            resultadoResumoJson: { results: results.length, sampleSize: take, modelName },
        },
    });
    return { ok: true, scanId: scan.id, results: createdResults.length };
}

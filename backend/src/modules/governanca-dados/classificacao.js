...
export async function aceitarSugestaoClassificacao(args) {
    ...
    if (!s || s.tenantId !== args.tenantId)
        return { ok: false, reason: 'SUGESTAO_INVALIDA' };
    if (s.statusSugestao !== 'PENDENTE')
...
export async function rejeitarSugestaoClassificacao(args) {
    const s = await prisma.governancaClassificacaoSugestao.findUnique({ where: { id: args.sugestaoId } }).catch(() => null);
    if (!s || s.tenantId !== args.tenantId)
        return { ok: false, reason: 'SUGESTAO_INVALIDA' };
    if (s.statusSugestao !== 'PENDENTE')

...
import { loginDto } from '@/modules/auth/application/dtos/loginDto.js'
import { registerDto } from '@/modules/auth/application/dtos/registerDto.js'
import { makeAuthUseCases } from '@/modules/auth/application/factories/makeAuthUseCases.js'
...
export default async function authRoutes(server: FastifyInstance) {
  ...
  // POST /register
  server.post('/register', { schema: { body: registerDto } }, async (request, reply) => {
    const ip = getClientIp(request.headers as Record<string, string>, (request as unknown as { ip: string }).ip)
    const now = Date.now()
    ...
    if (email) {
      const rl = peekRateLimit({ key: `register:email:${email}`, limit: 10, windowMs: 24 * 60 * 60 * 1000, now })
      if (!rl.ok) return reply.code(429).send({ message: 'Limite diário atingido para este e-mail.' })
    }
    if (cpf) {
      const rl = peekRateLimit({ key: `register:cpf:${cpf}`, limit: 4, windowMs: 24 * 60 * 60 * 1000, now })
      if (!rl.ok) return reply.code(429).send({ message: 'Limite de trial atingido para este CPF.' })
    }
    ...
    try {
      ...
      if (typeof body.googleToken === 'string' && body.googleToken.length > 0) {
        const payload = (server.jwt as unknown as { verify: (t: string) => Record<string, unknown> }).verify(body.googleToken)
      ...
      return reply.code(201).send({ message: 'User registered successfully', tenant: result.tenant, user: result.user })
    } catch (err) {
      addRateLimitHit({ key: `register:ip:${ip}`, windowMs: 60 * 60 * 1000, now })
      if (email) addRateLimitHit({ key: `register:email:${email}`, windowMs: 24 * 60 * 60 * 1000, now })
      if (cpf) addRateLimitHit({ key: `register:cpf:${cpf}`, windowMs: 24 * 60 * 60 * 1000, now })
  ...
  server.get('/google/start', async (_request, reply) => {
    const { GOOGLE_CLIENT_ID: clientId, GOOGLE_REDIRECT_URI: redirectUri } = process.env
    if (!clientId || !redirectUri) return reply.code(500).send({ message: 'Google OAuth não configurado' })
    const state = (server.jwt as unknown as { sign: (p: unknown, o: unknown) => string }).sign({ nonce: crypto.randomUUID() }, { expiresIn: '10m' })
  ...
  server.get('/google/callback', async (request, reply) => {
    const { GOOGLE_CLIENT_ID: clientId, GOOGLE_CLIENT_SECRET: clientSecret, GOOGLE_REDIRECT_URI: redirectUri, PUBLIC_APP_URL: appUrl } = process.env
    ...
    try { (server.jwt as unknown as { verify: (t: string) => unknown }).verify(state) }
    catch { return reply.code(401).send({ message: 'State inválido' }) }
    ...
    if (!tokenRes.ok) return reply.code(401).send({ message: 'Falha no Google OAuth' })
    ...
    const userinfo = await userinfoRes.json().catch(() => null) as Record<string, unknown> | null
    ...
    if (!email || !sub || !userinfo?.email_verified) return reply.code(401).send({ message: 'Conta Google inválida' })

    const googleToken = (server.jwt as unknown as { sign: (p: unknown, o: unknown) => string }).sign({ email, name, sub }, { expiresIn: '10m' })
    ...
    return reply.redirect(`${redirect}&${exists ? 'googleLogin=1' : 'mode=register'}`)
  })

  // POST /google/login
  server.post('/google/login', { schema: { body: z.object({ googleToken: z.string().min(10) }) } }, async (request, reply) => {
    try {
      const { googleToken } = request.body as { googleToken: string }
      const payload = (server.jwt as unknown as { verify: (t: string) => Record<string, unknown> }).verify(googleToken)
      const email = String(payload?.email || '')
      if (!email) return reply.code(401).send({ message: 'Token inválido' })
      const result = await useCases.login.execute({ email, password: '' }, server)
  ...
  // POST /login
  server.post('/login', { schema: { body: loginDto }, preHandler: [
    async (request, reply) => {
      const ip = getClientIp(request.headers as Record<string, string>, (request as unknown as { ip: string }).ip)
      const now = Date.now()
      const rl = checkRateLimit({ key: `login:ip:${ip}`, limit: 25, windowMs: 10 * 60 * 1000, now })
      if (!rl.ok) return reply.code(429).send({ message: 'Muitas tentativas. Tente novamente mais tarde.' })
      const email = normalizeEmail((request.body as Record<string, string>)?.email ?? '')
      if (email) {
    ...
    try {
      const result = await useCases.login.execute(request.body as z.infer<typeof loginDto>, server)
      return reply.send(result)
    } catch (err) {
      return replyError(reply, err)
    }
  ...
  // POST /select-tenant
  server.post('/select-tenant', { schema: { body: z.object({ userId: z.number(), tenantId: z.number() }) } }, async (request, reply) => {
    try {
      const { userId, tenantId } = request.body as { userId: number; tenantId: number }
      const result = await useCases.selectTenant.execute(userId, tenantId, server)
      return reply.send(result)
    } catch (err) {
      return replyError(reply, err)
    }
  ...
  server.put('/change-password', {
    ...
    schema: {
      body: z.object({
        ...
        newPassword: z.string().min(8).regex(/^(?=.*[A-Za-z])(?=.*\d).+$/, { message: 'Senha deve conter pelo menos 1 letra e 1 número' }),
      }),
    },
  }, async (request, reply) => {
    try {

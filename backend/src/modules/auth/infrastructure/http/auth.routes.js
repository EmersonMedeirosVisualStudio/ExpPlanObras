...
export default async function authRoutes(server) {
    ...
    server.post('/register', { schema: { body: registerDto } }, async (request, reply) => {
        ...
        if (email) {
            ...
            if (!rl.ok)
                return reply.code(429).send({ message: 'Limite diário atingido para este e-mail.' });
        }
        if (cpf) {
            const rl = peekRateLimit({ key: `register:cpf:${cpf}`, limit: 4, windowMs: 24 * 60 * 60 * 1000, now });
            if (!rl.ok)
                return reply.code(429).send({ message: 'Limite de trial atingido para este CPF.' });
        }
        if (process.env.HCAPTCHA_SECRET) {
            ...
            if (!captchaToken) {
                ...
                if (email)
                    addRateLimitHit({ key: `register:email:${email}`, windowMs: 24 * 60 * 60 * 1000, now });
                if (cpf)
                    addRateLimitHit({ key: `register:cpf:${cpf}`, windowMs: 24 * 60 * 60 * 1000, now });
        ...
        try {
            ...
            if (typeof body.googleToken === 'string' && body.googleToken.length > 0) {
                const payload = server.jwt.verify(body.googleToken);
    ...
    server.get('/google/callback', async (request, reply) => {
        ...
        if (!appUrl)
            return reply.code(500).send({ message: 'PUBLIC_APP_URL não configurado' });
        if (!code || !state)
    ...
    server.post('/google/login', { schema: { body: z.object({ googleToken: z.string().min(10) }) } }, async (request, reply) => {
        try {
            ...
            const payload = server.jwt.verify(googleToken);
    ...
    server.post('/login', { schema: { body: loginDto }, preHandler: [
            async (request, reply) => {
                ...
                if (email) {
                    ...
                    if (!rlEmail.ok)
                        return reply.code(429).send({ message: 'Muitas tentativas para este e-mail. Tente novamente mais tarde.' });
                }
            },
        ] }, async (request, reply) => {
        try {
            const result = await useCases.login.execute(request.body, server);
            return reply.send(result);
        }
        catch (err) {
            return replyError(reply, err);

...
describe('LoginUseCase', () => {
    ...
    it('throws InvalidCredentialsError when user not found', async () => {
        ...
        await expect(useCase.execute({ email: 'notfound@test.com', password: '123' }, mockApp)).rejects.toThrow(InvalidCredentialsError);
    });
    it('throws InvalidCredentialsError when password is wrong', async () => {
        vi.mocked(userRepo.findByEmail).mockResolvedValue(mockUser);
        await expect(useCase.execute({ email: mockUser.email, password: 'wrongpassword' }, mockApp)).rejects.toThrow(InvalidCredentialsError);
    });

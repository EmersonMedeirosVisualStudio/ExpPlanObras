...
export async function aplicarLegalHoldEmItem(args) {
    ...
    if (!item || item.tenantId !== args.tenantId)
        return { ok: false, reason: 'ITEM_INVALIDO' };
...
export async function liberarLegalHold(args) {
    const hold = await prisma.governancaLegalHold.findUnique({ where: { id: args.legalHoldId } }).catch(() => null);
    if (!hold || hold.tenantId !== args.tenantId)
        return { ok: false, reason: 'HOLD_INVALIDO' };
...
export async function aplicarLegalHoldPorCriteria(args) {
    const recurso = args.criteriaJson?.recurso ? String(args.criteriaJson.recurso).toUpperCase() : null;
    const entidadeIds = Array.isArray(args.criteriaJson?.entidadeIds) ? args.criteriaJson.entidadeIds.map((n) => Number(n)).filter((n) => Number.isFinite(n)) : null;
    ...
    if (entidadeIds && entidadeIds.length)
        where.entidadeId = { in: entidadeIds };
    if (statusRetencao)
        where.statusRetencao = statusRetencao;

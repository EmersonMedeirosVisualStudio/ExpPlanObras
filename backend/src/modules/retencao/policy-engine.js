...
function addPeriod(base, valor, unidade) {
...
export async function escolherPoliticaAtiva(args) {
...
export async function sincronizarItemRetencao(args) {
    ...
    const now = new Date();
    let elegivelDescarteEm = null;
    let elegivelArquivamentoEm = null;
    ...
    if (policy) {
        ...
        if (String(policy.acaoFinal || '').toUpperCase().includes('ARQUIVAR'))
            elegivelArquivamentoEm = elig;
    }
    ...
    if (holdAtivo)
        statusRetencao = 'EM_HOLD';
    else if (elegivelDescarteEm && elegivelDescarteEm <= now)
        statusRetencao = 'ELEGIVEL_DESCARTE';
    ...
    return { ok: true, itemId: saved.id, statusRetencao };
}

...
export async function simularDescarte(args) {
    const where = { tenantId: args.tenantId };
    if (args.filtro?.recurso)
        where.recurso = String(args.filtro.recurso).toUpperCase();
    if (args.filtro?.elegivelAte)
        where.elegivelDescarteEm = { lte: args.filtro.elegivelAte };
    if (!args.filtro?.incluirHold)
...
export async function aprovarLote(args) {
    const lote = await prisma.governancaDescarteLote.findUnique({ where: { id: args.loteId } }).catch(() => null);
    if (!lote || lote.tenantId !== args.tenantId)
        return { ok: false, reason: 'LOTE_INVALIDO' };
    if (String(lote.tipoExecucao) !== 'REAL')
        return { ok: false, reason: 'LOTE_NAO_REAL' };
    if (String(lote.statusLote) !== 'AGUARDANDO_APROVACAO')
...
export async function executarLote(args) {
    const lote = await prisma.governancaDescarteLote.findUnique({ where: { id: args.loteId } }).catch(() => null);
    if (!lote || lote.tenantId !== args.tenantId)
        return { ok: false, reason: 'LOTE_INVALIDO' };
    if (String(lote.tipoExecucao) !== 'REAL')
        return { ok: false, reason: 'LOTE_NAO_REAL' };
    if (String(lote.statusLote) !== 'APROVADO')
        return { ok: false, reason: 'LOTE_NAO_APROVADO' };
    ...
    for (const li of loteItems) {
        if (li.statusItem === 'BLOQUEADO_HOLD')
            continue;
        ...
        try {
            ...
            if (!handler)
                throw new Error('RECURSO_NAO_SUPORTADO');
            if (action === 'DELETE_HARD') {
                if (!handler.deleteHard)
                    throw new Error('DELETE_HARD_NAO_SUPORTADO');
                ...
                if (!handler.deleteSoft)
                    throw new Error('DELETE_SOFT_NAO_SUPORTADO');
                ...
                if (!handler.anonymize)
                    throw new Error('ANONIMIZAR_NAO_SUPORTADO');
                ...
                if (handler.archiveCold)
                    await handler.archiveCold(args.tenantId, li.entidadeId);
                await prisma.governancaRetencaoItem.update({
                    where: { id: li.retencaoItemId },
    ...
    return { ok: true, totalAnonimizados, totalDescartados, totalExpurgados, totalErros };
}

...
async function requireRetencaoAdmin(request, reply) {
...
export default async function retencaoRoutes(server) {
    ...
    server.post('/inventario/sincronizar', {
        schema: {
            body: z.object({
    ...
    server.get('/politicas', async (request, reply) => {
        ...
        if (q.recurso)
            where.recurso = normResource(q.recurso);
        if (q.ativo === 'true')
            where.ativo = true;
        if (q.ativo === 'false')
            where.ativo = false;
    ...
    server.put('/politicas/:id', {
        ...
        if (!current || current.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Política não encontrada');
    ...
    server.get('/inventario', async (request, reply) => {
        ...
        if (q.recurso)
            where.recurso = normResource(q.recurso);
        if (q.status)
            where.statusRetencao = String(q.status).toUpperCase();
        if (q.holdAtivo === 'true')
            where.holdAtivo = true;
        if (q.holdAtivo === 'false')
            where.holdAtivo = false;
        if (q.elegivel === 'true')
            where.elegivelDescarteEm = { lte: new Date() };
        if (q.elegivel === 'false')
            where.OR = [{ elegivelDescarteEm: null }, { elegivelDescarteEm: { gt: new Date() } }];
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.post('/legal-holds/:id/aplicar', { schema: { params: z.object({ id: z.coerce.number().int() }), body: z.object({ retencaoItemId: z.number().int() }) } }, async (request, reply) => {
        ...
        if (!hold || hold.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Hold não encontrado');
        ...
        if (!r.ok)
            return fail(reply, 400, r.reason);
    ...
    server.get('/descarte/lotes/:id', async (request, reply) => {
        ...
        if (!lote || lote.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Lote não encontrado');
    ...
    server.get('/auditoria', async (request, reply) => {
        ...
        if (q.recurso)
            where.recurso = normResource(q.recurso);
        if (q.tipoEvento)
            where.tipoEvento = String(q.tipoEvento).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([

...
export default async function continuidadeRoutes(server) {
    ...
    server.get('/planos', async (request, reply) => {
        ...
        if (q.tipo)
            where.tipoPlano = String(q.tipo).toUpperCase();
        if (q.ativo)
    ...
    server.post('/dr/execucoes', {
        ...
        if (!plano || plano.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Plano inválido');
    ...
    server.post('/dr/execucoes/:id/aprovar', async (request, reply) => {
        ...
        if (!ex || ex.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Execução não encontrada');
        if (ex.statusExecucao !== 'PENDENTE_APROVACAO')
            return fail(reply, 400, 'Status inválido');
        await prisma.drExecucaoRecuperacao.update({ where: { id: ex.id }, data: { statusExecucao: 'EXECUTANDO', aprovadoPor: ctx.userId, iniciadoEm: new Date() } });
        return ok(reply, { ok: true }, { message: 'Aprovado' });
    });
    server.post('/dr/execucoes/:id/concluir', {
        schema: {
        ...
        await prisma.drExecucaoRecuperacao.update({
            where: { id: ex.id },
            data: { statusExecucao: status, finalizadoEm: new Date(), rtoRealMinutos: body.rtoRealMinutos ?? null, rpoRealMinutos: body.rpoRealMinutos ?? null, resultadoResumoJson: body.resultadoResumoJson ?? null },
        });
    ...
    server.get('/crises', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusCrise = String(q.status).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([

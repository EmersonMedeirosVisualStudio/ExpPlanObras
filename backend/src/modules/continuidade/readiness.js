...
export async function calcularReadinessPlano(args) {
    ...
    if (!plano || plano.tenantId !== args.tenantId)
    ...
    if (runbooks > 0)
        score += 15;
    if (plano.ownerUserId)
        score += 10;
    if (plano.rtoMinutos > 0 && plano.rpoMinutos > 0)
        score += 10;
    if (ultTeste?.scoreProntidao && ultTeste.scoreProntidao >= 70)
        score += 20;
    if (drOk > 0)
        score += 20;
    if (plano.aprovadoPor && plano.aprovadoEm)
        score += 10;
    const comp = {
    ...
    return { ok: true, score, class: grade(score), componentes: comp };
}

...
function getAuthContext(request) {
    ...
    if (typeof userId !== 'number')
        return null;
    if (isSystemAdmin)
        return { tenantId: typeof tenantId === 'number' ? tenantId : null, userId, role: typeof role === 'string' ? role : 'SYSTEM_ADMIN', isSystemAdmin: true };
    if (typeof tenantId !== 'number')
        return null;
    return { tenantId, userId, role: typeof role === 'string' ? role : 'USER', isSystemAdmin: false };
}
async function requireGrcAdmin(request, reply) {
...
export default async function grcRoutes(server) {
    ...
    server.get('/riscos', async (request, reply) => {
        ...
        if (q.status)
            where.statusRisco = String(q.status).toUpperCase();
        if (q.categoria)
            where.categoriaRisco = String(q.categoria).toUpperCase();
    ...
    server.put('/riscos/:id', {
        ...
        if (!risco || risco.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Risco não encontrado');
    ...
    server.post('/riscos/:id/avaliacoes', {
        schema: {
            params: z.object({ id: z.coerce.number().int() }),
            body: z.object({
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (body.aplicarComoResidual !== false) {
                await tx.grcRisco.update({
                    where: { id: risco.id },
                    data: { impactoResidual: String(body.impacto), probabilidadeResidual: String(body.probabilidade), scoreResidual: score },
                });
            }
        });
        await emitObservabilityEvent({
    ...
    server.post('/riscos/:id/recalcular', async (request, reply) => {
        ...
        await prisma.grcRisco.update({
            where: { id: risco.id },
            data: { scoreResidual: residual, impactoResidual: risco.impactoResidual ?? risco.impactoInerente, probabilidadeResidual: risco.probabilidadeResidual ?? risco.probabilidadeInerente },
        });
    ...
    server.get('/controles', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.ativo)
            where.ativo = String(q.ativo).toLowerCase() === 'true';
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.post('/controles/:id/testes', {
        schema: {
            params: z.object({ id: z.coerce.number().int() }),
            body: z.object({
        ...
        if (!ctrl || ctrl.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Controle não encontrado');
    ...
    server.post('/controles/:id/riscos', {
        schema: {
            params: z.object({ id: z.coerce.number().int() }),
            body: z.object({
        ...
        await prisma.grcRiscoControle.create({
            data: { tenantId: ctx.tenantId, riscoId: risco.id, controleId: ctrl.id, papelControle: String(body.papelControle), pesoMitigacao: Number(body.pesoMitigacao) },
        });
    ...
    server.get('/auditorias', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusAuditoria = String(q.status).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.post('/auditorias/:id/encerrar', async (request, reply) => {
        ...
        if (!ctx || ctx.success === false)
            return;
        const { id } = z.object({ id: z.coerce.number().int() }).parse(request.params || {});
        const body = z.object({ opiniaoFinal: z.string().optional().nullable(), ratingFinal: z.string().optional().nullable() }).parse(request.body || {});
        const aud = await prisma.grcAuditoria.findUnique({ where: { id } }).catch(() => null);
        if (!aud || aud.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Auditoria não encontrada');
        await prisma.grcAuditoria.update({
            where: { id: aud.id },
            data: { statusAuditoria: 'ENCERRADA', dataFimReal: new Date(), opiniaoFinal: body.opiniaoFinal ?? null, ratingFinal: body.ratingFinal ?? null },
        });
    ...
    server.get('/achados', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusAchado = String(q.status).toUpperCase();
        if (q.gravidade)
            where.gravidade = String(q.gravidade).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.post('/achados/:id/encerrar', async (request, reply) => {
        ...
        if (!ach || ach.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Achado não encontrado');
        await prisma.grcAchado.update({ where: { id: ach.id }, data: { statusAchado: 'ENCERRADO' } });
    ...
    server.get('/planos-acao', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusPlano = String(q.status).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.post('/planos-acao/:id/aprovar', async (request, reply) => {
        ...
        if (!plano || plano.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Plano não encontrado');
        await prisma.grcPlanoAcao.update({ where: { id: plano.id }, data: { statusPlano: 'EM_ANDAMENTO', aprovadorUserId: ctx.userId } });
    ...
    server.post('/planos-acao/:id/concluir', async (request, reply) => {
        ...
        if (!plano || plano.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Plano não encontrado');
        await prisma.grcPlanoAcao.update({ where: { id: plano.id }, data: { statusPlano: 'CONCLUIDO', concluidoEm: new Date() } });
    ...
    server.get('/evidencias', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.referenciaTipo)
            where.referenciaTipo = String(q.referenciaTipo).toUpperCase();
        if (q.referenciaId)
            where.referenciaId = Number(q.referenciaId);
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([

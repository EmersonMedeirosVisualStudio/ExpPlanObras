...
export default async function observabilidadeRoutes(server) {
    ...
    server.get('/eventos', async (request, reply) => {
        ...
        if (q.categoria)
            where.categoria = String(q.categoria).toUpperCase();
        if (q.severidade)
            where.severidade = String(q.severidade).toUpperCase();
        if (q.resultado)
            where.resultado = String(q.resultado).toUpperCase();
        if (q.origemTipo)
            where.origemTipo = String(q.origemTipo).toUpperCase();
        const filters = [];
        if (q.texto) {
        ...
        if (q.desde || q.ate) {
            where.ocorridoEm = {};
            if (q.desde)
                where.ocorridoEm.gte = new Date(q.desde);
            if (q.ate)
                where.ocorridoEm.lte = new Date(q.ate);
        }
    ...
    server.get('/eventos/:id', async (request, reply) => {
        ...
        if (!ev || ev.tenantId !== ctx.tenantId)
            return fail(reply, 404, 'Evento não encontrado');
        return ok(reply, {
            id: ev.id,
            eventId: ev.eventId,
            ...
            payloadRedacted: ev.payloadRedactedJson,
            labels: ev.labelsJson,
    ...
    server.get('/alertas', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusAlerta = String(q.status).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([
    ...
    server.get('/incidentes', async (request, reply) => {
        ...
        const where = { tenantId: ctx.tenantId };
        if (q.status)
            where.statusIncidente = String(q.status).toUpperCase();
        const skip = (q.pagina - 1) * q.limite;
        const [rows, total] = await Promise.all([

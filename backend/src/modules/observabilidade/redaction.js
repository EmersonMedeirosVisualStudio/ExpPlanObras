...
function redactValueByKey(key, value) {
    ...
    if (k.includes('password') || k.includes('senha'))
        return 'REDACTED';
    if (k.includes('token') || k.includes('secret') || k.includes('authorization') || k.includes('callback'))
        return 'REDACTED';
    if (k.includes('cpf'))
        return typeof value === 'string' ? maskCpf(value) : 'REDACTED';
    if (k.includes('cnpj'))
        return typeof value === 'string' ? maskCnpj(value) : 'REDACTED';
    if (k.includes('email'))
        return typeof value === 'string' ? maskEmail(value) : 'REDACTED';
    if (k.includes('telefone') || k.includes('phone') || k.includes('celular'))
        return typeof value === 'string' ? maskPhone(value) : 'REDACTED';
    if (k.includes('medico') || k.includes('saude'))
        return 'REDACTED';
    return null;
}
export function redactPayload(input) {
    ...
    if (Array.isArray(input))
        return input.map((it) => redactPayload(it));

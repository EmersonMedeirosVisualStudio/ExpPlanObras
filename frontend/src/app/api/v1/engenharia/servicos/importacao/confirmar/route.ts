...
import { previewServicos } from '@/lib/modules/engenharia-importacao/validators';
...
export async function POST(req: NextRequest) {
  try {
    const current = await requireApiPermission(PERMISSIONS.ENGENHARIA_SERVICOS_IMPORTAR);
    ...
    const preview = previewServicos(csvText);
    ...
    try {
      ...
      for (const r of rows) {
        ...
        const [res]: any = await conn.query(
          `
          INSERT INTO engenharia_servicos
            (tenant_id, codigo, descricao, unidade, grupo, preco_unitario, ativo)
          VALUES
            (?,?,?,?,?,?,?)

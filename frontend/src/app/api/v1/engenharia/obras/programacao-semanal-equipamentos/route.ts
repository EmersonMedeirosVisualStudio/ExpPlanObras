...
export async function GET(req: NextRequest) {
  try {
    ...
    const [rows]: any = await db.query(
      `
      SELECT
        id_programacao_equip AS idProgramacao,
        id_obra AS idObra,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [[existing]]: any = await conn.query(
      `SELECT id_programacao_equip AS idProgramacao FROM engenharia_programacoes_semanais_equipamentos WHERE tenant_id = ? AND id_obra = ? AND semana_inicio = ? LIMIT 1`,
    ...
    const [ins]: any = await conn.query(
      `
      INSERT INTO engenharia_programacoes_semanais_equipamentos

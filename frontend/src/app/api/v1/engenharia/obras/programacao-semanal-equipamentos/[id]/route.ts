...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [[head]]: any = await db.query(
      `
      SELECT id_programacao_equip AS idProgramacao, id_obra AS idObra, semana_inicio AS semanaInicio, semana_fim AS semanaFim, status
      FROM engenharia_programacoes_semanais_equipamentos
      WHERE tenant_id = ? AND id_programacao_equip = ?
      LIMIT 1
      `,
    ...
    const [itens]: any = await db.query(
      ...
      SELECT
        ...
        horas_previstas AS horasPrevistas,
        frente_trabalho AS frenteTrabalho,
        observacao
      FROM engenharia_programacoes_semanais_equipamentos_itens
      WHERE tenant_id = ? AND id_programacao_equip = ?
      ORDER BY data_referencia ASC, id_ativo ASC, codigo_servico ASC
      `,
      [current.tenantId, idProgramacao]
    );
    ...
    return ok({
      ...
      itens: (itens as any[]).map((r) => ({
        ...
        horasPrevistas: r.horasPrevistas == null ? null : Number(r.horasPrevistas),
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [[head]]: any = await conn.query(
      `SELECT id_obra AS idObra, status FROM engenharia_programacoes_semanais_equipamentos WHERE tenant_id = ? AND id_programacao_equip = ? LIMIT 1`,
      [current.tenantId, idProgramacao]
    );
    if (!head) return fail(404, 'Programação não encontrada');
    if (!canAccessObra(current as any, Number(head.idObra))) return fail(403, 'Sem acesso à obra');
    ...
    for (const it of itens) {
      ...
      const idAtivo = Number(it?.idAtivo || 0);
      const codigoServico = String(it?.codigoServico || '').trim().toUpperCase();
      if (!dataReferencia || !idAtivo || !codigoServico) continue;

      const horasPrevistas = it?.horasPrevistas == null ? null : toNumber(it.horasPrevistas);
      ...
      await conn.query(
        `
        INSERT INTO engenharia_programacoes_semanais_equipamentos_itens
          (tenant_id, id_programacao_equip, data_referencia, id_ativo, codigo_servico, horas_previstas, frente_trabalho, observacao)
        VALUES
          (?,?,?,?,?,?,?,?)
        ...
        [
          ...
          idAtivo,
          codigoServico,
          horasPrevistas == null || Number.isNaN(horasPrevistas) ? null : horasPrevistas,
          frenteTrabalho,
          observacao,
        ]
      );

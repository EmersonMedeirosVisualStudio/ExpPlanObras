...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [[row]]: any = await db.query(`SELECT COUNT(*) AS total FROM obras_planilhas_itens WHERE tenant_id = ? AND id_obra = ?`, [current.tenantId, idObra]);
    const total = Number(row?.total || 0);
    return ok({ idObra, totalItens: total, possuiPlanilha: total > 0 });
  } catch (e) {

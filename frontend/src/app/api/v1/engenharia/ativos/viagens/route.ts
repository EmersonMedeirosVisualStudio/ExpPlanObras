...
async function ensureTables() {
  await db.query(
    `
    CREATE TABLE IF NOT EXISTS engenharia_viagens_caminhao (
  ...
  try {
    ...
    const [rows]: any = await db.query(
      ...
      SELECT
        ...
        km,
        observacao
      FROM engenharia_viagens_caminhao
      WHERE ${where.join(' AND ')}
      ORDER BY data_referencia DESC, id_viagem DESC
      LIMIT 500
      `,
      params
    );
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const tipoCarga = body?.tipoCarga ? String(body.tipoCarga).trim() : null;
    const km = toNumberOrNull(body?.km);
    ...
    if (!destino) return fail(422, 'destino é obrigatório');
    if (km != null && km < 0) return fail(422, 'km inválido');
    ...
    const [ins]: any = await conn.query(
      `
      INSERT INTO engenharia_viagens_caminhao
        (tenant_id, id_ativo, data_referencia, tipo_local, id_local, codigo_servico, origem, destino, tipo_carga, km, observacao, id_funcionario_lancador)
      VALUES
        (?,?,?,?,?,?,?,?,?,?,?,?)
      `,
      [current.tenantId, idAtivo, dataReferencia, tipoLocal, idLocal, codigoServico, origem, destino, tipoCarga, km, observacao, current.idFuncionario ?? null]
    );
    await conn.commit();
    return ok({ idViagem: Number(ins.insertId) });
  } catch (e) {
    await conn.rollback();
    return handleApiError(e);
  } finally {

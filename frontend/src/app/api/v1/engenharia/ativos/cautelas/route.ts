...
export async function POST(req: NextRequest) {
  try {
    ...
    const [ins]: any = await db.query(
      `
      INSERT INTO engenharia_cautelas (tenant_id, tipo_local, id_local, data_referencia, status, id_funcionario_responsavel, observacao)
      VALUES (?, ?, ?, ?, 'ABERTA', ?, ?)
    ...
    const [[row]]: any = await db.query(
      `SELECT id_cautela AS idCautela FROM engenharia_cautelas WHERE tenant_id = ? AND tipo_local = ? AND id_local = ? AND data_referencia = ? LIMIT 1`,

...
import { duplicarModeloComoRascunho } from '@/lib/modules/workflows-designer/server';
...
export async function POST(_req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const data = await duplicarModeloComoRascunho(current.tenantId, current.id, modeloId);
    return ok(data);
  } catch (e) {

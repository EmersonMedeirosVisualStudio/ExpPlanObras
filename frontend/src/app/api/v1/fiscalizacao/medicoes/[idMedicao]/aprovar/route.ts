...
export async function POST(_req: NextRequest, ctx: { params: Promise<{ idMedicao: string }> }) {
  ...
  try {
    ...
    if (!['ENVIADA', 'EM_ELABORACAO'].includes(status)) return fail(422, 'Status não permite aprovação');
    if (String(row.origem || 'ENGENHEIRO') === 'ENGENHEIRO' && status !== 'ENVIADA') {
      return fail(422, 'Medição do engenheiro deve ser enviada antes de aprovar');
    }

    await conn.beginTransaction();
    await conn.query(
      `UPDATE contratos_medicoes SET status_medicao = 'APROVADA' WHERE tenant_id = ? AND id_medicao = ?`,
      [current.tenantId, idMedicao]
    );
    await conn.query(
      `UPDATE contratos_medicoes_detalhes SET aprovado_em = NOW(), id_usuario_aprovador = ? WHERE tenant_id = ? AND id_medicao = ?`,
      [current.id, current.tenantId, idMedicao]
    );
    ...
    await audit({
      ...
      acao: 'APROVAR_FISCAL',
      dadosNovos: { idObra: Number(row.idObra), origem: String(row.origem || 'ENGENHEIRO') },
    });

    await conn.commit();
    return ok({ idMedicao, status: 'APROVADA' });
  } catch (e) {
    await conn.rollback();

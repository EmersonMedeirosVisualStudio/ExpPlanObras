...
import { obterMatrizCentroExecutivo, parseCentroExecutivoFiltrosFromRequest } from '@/lib/modules/centro-executivo/server';
...
export async function GET(req: Request) {
  try {
    ...
    const data = await obterMatrizCentroExecutivo({ tenantId: current.tenantId, userId: current.id, permissions }, filtros);
    return ok(data);
  } catch (e) {

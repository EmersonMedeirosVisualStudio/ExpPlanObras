...
import { assertPortalScopeHasLocal, obterResumoPortalGestor } from '@/lib/modules/portal-gestor/server';
...
export async function GET(req: Request) {
  try {
    ...
    const data = await obterResumoPortalGestor({ tenantId: current.tenantId, userId: current.id, scope, filtros });
    return ok(data);
  } catch (e) {

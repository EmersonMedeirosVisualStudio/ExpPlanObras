...
export const DashboardGerenteApi = {
  ...
  resumo: (f?: { idObra?: number | null; idUnidade?: number | null }) => api<any>(`/api/v1/dashboard/gerente/resumo${qs(f)}`),
  alertas: (f?: { idObra?: number | null; idUnidade?: number | null }) => api<any[]>(`/api/v1/dashboard/gerente/alertas${qs(f)}`),
  obterLayout: () => api<any>('/api/v1/dashboard/me/layout?dashboard=GERENTE'),

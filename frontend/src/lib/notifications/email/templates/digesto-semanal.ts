...
export function buildDigestoSemanalTemplate(input: EmailTemplateBuildInput): EmailTemplateBuildOutput {
  const itens = input.itensDigesto || [];
  const assunto = `Digesto semanal de notificações`;
